...
| `knowgrph-game-flight-sim-demo.md` | `flight-sim` | XR Mode overlay on the Physics-authored world | local-runtime-ready | Native deterministic flight, shared Fixed Follow / Free Orbit Camera catalog and Physics controller camera, spec-primary aircraft, strict browser-local invocation, Decisions-only WorkspaceFs; prove with `npm run game-flight-sim:runtime-ready` and `npm run game-flight-sim:browser-smoke` |
