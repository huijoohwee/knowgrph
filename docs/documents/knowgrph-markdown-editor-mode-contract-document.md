...
- Viewer header controls must be actionable-only: hide Content controls when no mode switch exists, hide `Node Quick Editor` when unavailable, and avoid disabled/no-op toolbar placeholders.
