...
- A direct provider-backed TextGeneration Widget Card Run keeps the authored prompt, summary, and other input fields on the source card. Streaming and terminal generated Markdown belong to one workflow-owned Rich Media Panel connected by the typed `text_out -> output` edge; repeated Run reuses that panel and edge instead of writing provider output back into the Widget Card. Explicit Rich Media targets still win, while Run All retains its separate no-implicit-panel gate.
