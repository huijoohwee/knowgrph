...
- **YouTube Transcript Import**: Workspace Actions → Source Files can convert a YouTube URL/ID into Markdown via [youtubeImportAction.ts](file:///Users/huijoohwee/Documents/GitHub/knowgrph/canvas/src/features/toolbar/youtubeImportAction.ts), which POSTs to the dev/preview endpoint in [vite.config.ts](file:///Users/huijoohwee/Documents/GitHub/knowgrph/canvas/vite.config.ts) (`/__youtube_transcript`). The endpoint shells out to `python3 -m knowgrph_parser youtube` to fetch transcripts and emit Slidev-compatible Markdown (using `---` separators), then the Canvas import path parses the Markdown and updates the Markdown Viewer/Presentation state.
...
| Ingestion | `features/toolbar` | `youtubeImportAction` | `performYouTubeImport` | Toolbar triggers transcript fetch → converts to Markdown → invokes parser → updates store | `vite dev hook`, `knowgrph_parser` | HTTP + CLI | ~140 |
