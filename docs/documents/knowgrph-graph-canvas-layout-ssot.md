...
- Radial orbit animation for 2D D3 is controlled by schema-only knobs `layout.forces.radialOrbit*` (enable, speedDeg, orbitSize, ringGapPx, depthSpeedScale, mode∈{flat,solar,atomic}); it runs as a view-only render-frame animator that never re-derives GraphData or restarts D3 simulation, and is gated to radial+D3, non-Bipartite graphs.
