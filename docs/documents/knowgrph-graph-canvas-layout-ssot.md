...
|---|---|---|
| Layout defaults (force/fit/collision/groups) | `canvas/src/lib/graph/layoutDefaults.ts` | Canonical numeric defaults + safe readers. Any fallback logic must live here (not spread across schema/editor/simulation). |
...
- Layout position caches must be isolated by the full key `(semanticMode, frontmatterMode, layoutMode, renderMode)` (no fallback to partial/legacy keys).
