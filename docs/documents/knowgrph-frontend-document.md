...
- Floating Panel hosts a dedicated `media` view to the right of `View`, and that view reuses the same Geo KTV-style lightweight layout contract as other structured FloatingPanel views instead of introducing a foreign command-palette shell. FloatingPanel `media` is the SSOT for current `@` image, audio, video, webpage, iframe, YouTube, and graph rich-media browsing; MainPanel Help → Command Menu owns the full `/`, `@`, and `#` catalog reference.
...
- The Floating Panel `media` view is the current `@` image, audio, video, webpage, iframe, YouTube, and graph rich-media list. It must reuse the shared media candidate resolver, feed Preview Panel selection without duplicating the gallery there, and must not display the full `/` command catalog.
