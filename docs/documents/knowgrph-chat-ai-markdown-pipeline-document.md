...
- GitGraph, Gantt, and Geospatial requests stay on the same neutral dataflow path. Mermaid diagram source, including typed `flow_diagrams` entries with `type: mermaid_gitgraph` or `type: mermaid_gantt`, and GeoJSON/FeatureCollection payloads under neutral `geoJson`/`geojson`/coordinate fields are data that feed source/card/widget -> safe compute -> Rich Media Panel `outputSrcDoc`; they must not be treated as static panel copies, renderer-local Timeline instructions, document version-control GitGraph state, or Geospatial Mode toggles.
...
- Optional typed diagram records such as `flow_diagrams.gitgraph.type: mermaid_gitgraph`, `flow_diagrams.gantt.type: mermaid_gantt`, and neutral geospatial records with `geoJson` / FeatureCollection payloads remain frontmatter or structured-content data inputs; rendered panels are generated through compute-backed Rich Media Panel output, not through static backfill.
