...
- BottomPanel Timeline transport and Media preview playback share the same document transport/controller path. In XR context, `XrTimelineSceneLane` feeds native stage, cast, camera, Scene, and Runtime tasks into the same `GanttTimelineTransportPanel`; the shared transport owns play/pause and scrub state, generated XR rows stay read-only, and FloatingPanel XR must not fork motion, Scene, or Runtime state.
