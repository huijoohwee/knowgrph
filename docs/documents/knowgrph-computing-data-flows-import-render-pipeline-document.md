...
- Typed `flow_diagrams` frontmatter is an ordinary source-owned input block. Entries such as `type: mermaid_gitgraph`, `type: mermaid_gantt`, `type: mermaid_architecture`, and `type: mermaid_eventmodeling` derive generic `FlowDiagramSource -> TextGeneration compute -> RichMediaPanel` dataflow nodes, preserving the typed frontmatter source while letting Flow Editor and Storyboard consume live `outputSrcDoc` through the shared inline compute runner. The parser must not inspect demo filenames, regenerate stale templates, or backfill static Rich Media Panel payloads.
- The same typed Mermaid diagram source is resolved for GitGraph/Gantt/Timeline/Architecture/Event Model BottomPanel previews through `mermaidDiagramCode.ts` and `MermaidDiagramPanelView.tsx`; Canvas View Display Controls open those BottomPanel tabs by writing `bottomSurfaceTab` / `bottomSurfaceCollapsed`, and panel rendering must reuse the shared Mermaid SVG renderer instead of creating downstream Gantt-specific render branches.
