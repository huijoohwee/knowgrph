...
- GraphTable/Multi-dimensional Table view state persistence (column visibility/order, filters, sort, row height, widths) uses a single `graph-table:view-state` scheduler key and LS keys (`kg:ui:graphTable:*`); persisted `GraphTableDb` sync remains revision+viewKey gated and independent of view-state persistence.
