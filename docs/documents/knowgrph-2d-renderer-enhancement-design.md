...
- Media canvas, BottomPanel `Timeline`, FloatingPanel `Timeline`, and FloatingPanel `Media` share `timelineAnimationEngine.ts` for native animation data attributes, CSS custom properties, object frame values, and SVG frame values. Anime.js target breadth is inspiration only: the implementation is dependency-free and exposes CSS property, SVG attribute, DOM attribute, JavaScript object, HTML, Canvas 2D, and WebGL/Three.js target semantics through in-repo runtime state.
