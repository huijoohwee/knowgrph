...
|---|---|---|
| FloatingPanel Media | Media browser/catalog | Lists uploaded and generated image/audio/video records, previews uploaded media inline inside FloatingPanel Media through the shared Rich Media Panel, downloads media through the shared download helper, and inserts selected media into active card fields as inline mention chips. The panel-local preview keeps parent-owned placement so Canvas Storyboard/frontmatter positioning cannot collapse image, video, or audio rendering, and it does not create a modal/lightbox route. Its thumbnails share hover/focus-appearing kind, info, open-link, and download overlays with Storyboard card media and reference thumbnails. |
