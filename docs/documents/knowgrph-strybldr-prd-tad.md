...
- Given a user opens the shared camera framing owner from FloatingPanel Camera or FloatingPanel XR, drags the camera handle around its sphere, and clicks `Reframe`, then camera orbit coordinates, angle, level, shot size, and optional note persist on the selected graph node as `strybldrCamera` and synchronize with the live Three camera without duplicating prompt-local or projection-local state.
- Given a user changes Camera framing, invokes an XR X/Y/Z view command, manually orbits the XR canvas, switches between FloatingPanel Camera and XR, or leaves and re-enters XR Mode, then both projections and the live Three camera converge on the same framing snapshot; reverse synchronization waits for OrbitControls damping to settle, repeated axis commands recenter, and `Reframe` is the only graph/history write.
...
| FloatingPanel Camera and XR Projections | `canvas/src/features/strybldr/StrybldrCameraFloatingPanelView.tsx`, `canvas/src/features/three/XrCameraFramingSection.tsx`, `canvas/src/features/three/XrPanelView.tsx` | Keep Camera and XR as distinct thin FloatingPanel shells that both mount the shared camera framing owner without duplicating the board model or graph update path; no BottomPanel XR route remains. |
...
| Camera geometry and sync | `strybldr.renderer.sharedSurfaceRegistry` focused camera checks | FloatingPanel Camera and XR mount one shared framing owner whose 3D degree-grid orbit vectors drive handle, meridian/latitude highlight, frame-aware ray polygon, shot alignment, and synchronized live Three camera poses. |
...
- FloatingPanel Camera and FloatingPanel XR are distinct thin projections over `StrybldrCameraFramingSection`; that shared owner alone reads and writes `strybldrCamera`, derives the selected storyboard card, synchronizes the live Three camera through `cameraFramingRuntime` and `cameraFramingPose`, and uses shared 3D degree-grid geometry. BottomPanel XR and duplicate projection-local board/update ownership are absent.
