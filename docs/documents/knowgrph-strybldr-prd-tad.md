...
- Given Storyboard cards expose Run, Update KV entry, Open sidepane, Convert to loop, Duplicate, Clear output, Help, or Remove, when the toolbar renders, then Storyboard reuses the shared Flow Editor toolbar surface plus shared Storyboard action, binding, presentation, and props helpers rather than rebuilding per-card action choreography or visual config inline.
...
| Storyboard Toolbar Reuse | `canvas/src/components/StoryboardCanvas/storyboard*Action*.ts`, `storyboardToolbarActionBindings.ts`, `storyboardToolbarPresentation.ts`, `storyboardToolbarProps.ts` | Reuse the shared Flow Editor toolbar surface while keeping Storyboard-specific action routing, card-scoped bindings, and presentation/visual props in neutral helpers instead of inline card-loop choreography. |
