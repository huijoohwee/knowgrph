...
- FloatingPanel Camera is the sole projection of `StrybldrCameraFramingSection`; it alone reads and writes `strybldrCamera`, derives the selected storyboard card, synchronizes the live Three camera through `cameraFramingRuntime` and `cameraFramingPose`, and uses shared 3D degree-grid geometry. FloatingPanel XR retains activation, graphics, Gaussian, and spatial tools without mounting camera or motion controls. BottomPanel Timeline reuses its existing player, playhead, and Scene/Effect lanes for XR motion authoring, camera-mark capture, Scene state, and Runtime state; a standalone XR form/player, dedicated BottomPanel XR, and duplicate camera, board, or update ownership are absent.
