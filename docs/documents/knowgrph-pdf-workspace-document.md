...
## Scope
Local-only, deterministic **PDF → Markdown → Render** pipeline that writes artifacts under the repo-local ignored `.knowgrph-workspace/…` root, and surfaces the workflow inside **MainPanel → Workflow Manager** (no dedicated `/workspace` or `/import` pages).
