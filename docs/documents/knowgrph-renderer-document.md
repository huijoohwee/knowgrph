...
- **2D vs 3D renderer parity**:
  - Apex Home presentation is renderer-independent: `CanvasViewport` mounts one full-viewport Live Canvas Hero owner whenever the route grants hero ownership, while its isolated iframe owns the selected 2D, 3D, or XR background. The parent workspace must not reserve an editor inset or mount a competing 2D surface, Three surface, or XR HUD until Home ownership ends; `/knowgrph` retains normal renderer ownership.
