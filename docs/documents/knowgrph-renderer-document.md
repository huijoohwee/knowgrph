...
- **2D vs 3D renderer parity**:
  ...
  - Flight Sim run-ready mode is an actor-and-HUD overlay on that same XR Mode surface. `FlightSimMissionStage` supplies the airplane and waypoint/objective rings, while `flightSimFollowTarget` supplies a pure aircraft follow/framing descriptor to the canonical `useXrNativeControllerDemoCamera` hook. That hook alone mutates the camera and OrbitControls. Fixed Follow / Free Orbit selection remains owned by `xrNativeControllerCameraCatalog` and `xrNativeControllerCameraRuntime`; Timeline playback suspends the hook and returns to the selected source afterward. Flight adds no camera driver, renderer, second rendered XR world or scene owner, terrain, or collider owner.
