...
- Animation renderer contract: `kgCanvas2dRenderer: "animation"` reuses the same canonical `flow:` frontmatter graph authoring surface as `kgCanvas2dRenderer: "flowEditor"` and may add `timeline.beats.*` timing metadata beside that shared graph contract; renderer switches and toolbar menu labels must derive from shared renderer helpers instead of duplicated inline renderer tables.
- **Renderer coverage**:
