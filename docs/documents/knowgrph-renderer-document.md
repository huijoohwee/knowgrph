...
- **2D vs 3D renderer parity**:
  ...
  - Game FPS run-ready mode is a source-backed 3D projection inside the existing `ThreeGraph` React Three Fiber Canvas. `GameFpsMissionStage` owns procedural scene meshes and first-person camera projection only while the canonical Game FPS seed is active; `GameFpsHud` owns keyboard, pointer, touch, mission, and save controls outside the WebGL tree. The mode suppresses ordinary graph meshes, overlays, minimap, OrbitControls, and competing camera requests instead of creating a second Canvas or renderer. Its fixed-step Agentic ECS world remains ephemeral, while validated mission Decisions persist through browser-local WorkspaceFs.
