...
- **Solution**: Canvas may **warm-mount** inactive renderers to reduce switch lag, but enforces strict **active gating**:
  ...
  - Caches remain SSOT-keyed: zoom state is restored via a stable view key that is shared across 2D renderer variants; layout caches remain isolated by render variant.
