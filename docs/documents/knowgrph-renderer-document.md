...
- Frontmatter contract split: canonical authored docs and templates keep `flow:` in plain YAML scalars, arrays, and objects, while normalized `{key, type, value}` wrappers stay limited to E2E validation fixtures that audit ingestion -> parsing -> rendering on Canvas.
