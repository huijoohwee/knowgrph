...
- **2D vs 3D renderer parity**:
  ...
  - Motion Control extends both graph-backed and physics-playground XR branches through one browser-local pose owner. `motionControlRuntime` requests camera access only after an explicit Start, loads the official app-served LiteRT Wasm runtime and checksum-pinned Google pose landmark model, runs at most one inference at a time, publishes one confidence-gated normalized pose, and releases media tracks, tensors, and model resources on stop, panel close, XR exit, or page hide. `MotionControlFloatingPanelView` reuses the shared FloatingPanel catalog layout and shows local preview/status only; `motionControlPose` projects the same pose into the selected humanoid and the native physics controller input before the single physics step. The standalone landmark model receives a centered 256×256 person crop and then a bounded prior-landmark ROI, so the UI instructs the operator to keep one full body centered and does not claim arbitrary-frame detection or multi-person tracking. Raw frames and pose history remain ephemeral and never enter graph persistence, MCP results, or network requests. [The Motion Control contract](knowgrph-motion-control-prd-tad.md) owns the official-source and no-copy boundaries.
