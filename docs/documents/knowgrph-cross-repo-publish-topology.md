...
| Release flow | Build and validate in `knowgrph` through `Integration Gate`. Only the manually dispatched `Production Release` workflow may sync the exact verified commit into an ephemeral `huijoohwee` artifact and deploy it through the protected `production` environment. Storage, D1, DNS, and payment deployments remain separate explicitly authorized operations. | `knowgrph/.github/workflows/release.yml` | `huijoohwee` release artifact | `airvio.co/knowgrph` |
...
| Live Canvas Hero parity | Open Dev and `https://airvio.co/` in parallel; verify the `Knowgrph Live Canvas Hero` and interactive canvas regions, confirm the hero's visible **Share canvas embed** action copies iframe HTML whose `src` carries `kgPreview=1&kgLiveHero=1`, then open that `src` and verify `data-kg-live-canvas-hero-embed-preview=true`, `data-kg-canvas-interactive=1`, and the source-derived Flow node/edge counts. In Explorer → Source Files, invoke **Share canvas embed** for a non-README source; confirm the clipboard contains the sandbox/referrer/fullscreen contract, `data-kg-live-canvas-hero-source` changes immediately, `data-kg-live-canvas-hero-selected-embed=true` mounts, and the iframe URL is same-origin with interactive controls/canvas surfaces. On both the apex and selected embed, assert zero visible Canvas toolbars, editor shells, Mermaid SVGs, timelines, and minimaps. | Confirms the public root renders the actual React canvas hero, the Explorer action swaps the underlying interactive source while producing a third-party iframe contract, and no persisted workspace renderer or shell surface seeps into the hero owner. |
