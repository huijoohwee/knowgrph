...
| Release flow | Build and validate in `knowgrph` through `Integration Gate`. Pull-request checks may supersede the same PR, while merge-group and protected-main checks are isolated by exact SHA. A protected green push to `main` automatically runs the sole serialized production controller, which pins the mirror base, syncs the exact verified commit into an ephemeral `huijoohwee` artifact, records managed deletions, preserves reviewed hidden readiness files, and reconciles that artifact into the same base before deployment through the protected `production` environment. The controller revalidates that its candidate is still the current remote `main` immediately before Direct Upload, immediately afterward, and before mirror publication, so a delayed stale push event cannot overwrite a newer release. Cloudflare Git-triggered deployments must be disabled. Storage, DNS, and payment deployments remain separate explicitly authorized operations. | `knowgrph/.github/workflows/{integration,release}.yml` | `huijoohwee` release artifact | `airvio.co/knowgrph` |
