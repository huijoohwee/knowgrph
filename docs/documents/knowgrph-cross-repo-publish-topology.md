...
| Generated chat storage | Keep FloatingPanel Chat KGC sessions under `/chat-log/{session}/`; promote Markdown/text artifacts to D1 canonical paths `chat-log/{session}/{file}` through Source Files storage, and store generated binary bytes in R2 with sibling Markdown manifests promoted to D1. | `knowgrph/canvas/src/features/{chat,source-files}`, `knowgrph/cloudflare/workers/knowgrph-storage` | Cloudflare Worker D1 rows + R2 `knowgrph-storage-blobs` | `airvio.co/api/storage/{doc,blob}/*` |
...
| Generated chat storage proof | `curl -i https://airvio.co/api/storage/doc/{workspaceId}/chat-log%2F{session}%2Fkgc_{session}.md` | Confirms promoted New Chat KGC Markdown is publicly readable from D1 after runtime sync. |
