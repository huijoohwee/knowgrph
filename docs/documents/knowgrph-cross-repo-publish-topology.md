...
| Root launch alias | Keep `airvio.co/` on the published Knowgrph React app shell. The generated root handler injects `x-knowgrph-root-alias=/knowgrph/`; Home renders the canonical workspace README's isolated **Share canvas embed** runtime, not a copied/static page or the full `/knowgrph/` workspace canvas. Explorer → Source Files → **Share canvas embed** selects and session-persists another source's same-origin `kgDoc` interactive runtime, replacing the canonical background. The later published URL replaces it only when same-origin, so localhost never embeds a Cloudflare frame that production security headers reject. | `knowgrph/cloudflare/pages/root-agent-ready-index.mjs`, `knowgrph/canvas/src/features/canvas/{useKnowgrphLiveCanvasHero,liveCanvasHeroSourceSelection,canvasDocDeepLink}.ts` | `huijoohwee/_worker.js`, `huijoohwee/knowgrph/**` | `airvio.co` → `airvio.co/knowgrph/` |
