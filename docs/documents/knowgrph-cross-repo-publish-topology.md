...
| Root launch alias | Keep `airvio.co/` on the published Knowgrph React app shell. The generated root handler injects `x-knowgrph-root-alias=/knowgrph/`; the normal hero must be the live `workspace-readme.md` canvas, not a copied/static launch page. If persisted workspace state does not materialize that source, the root hook fetches the canonical `/docs/workspace-readme.md` text and parses the same FlowCanvas projection. Explorer → Source Files → **Share canvas embed** immediately swaps the in-session hero to the selected source's same-origin `kgDoc` interactive runtime through the shared selection event. The later published URL replaces it only when same-origin, so localhost never embeds a Cloudflare frame that production security headers reject. | `knowgrph/cloudflare/pages/root-agent-ready-index.mjs`, `knowgrph/canvas/src/features/canvas/{useKnowgrphLiveCanvasHero,liveCanvasHeroSourceSelection,canvasDocDeepLink}.ts` | `huijoohwee/_worker.js`, `huijoohwee/knowgrph/**` | `airvio.co` → `airvio.co/knowgrph/` |
