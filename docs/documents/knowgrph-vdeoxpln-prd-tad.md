...
| Visual annotation renderer reuse contract | `canvas/src/lib/config.render.ts` | Declares `2D Renderer: Flow Editor`, `2D Renderer: Media`, and `2D Renderer: Storyboard` as the only renderer surfaces that reuse the visual annotation E2E runtime. | Treat external CV projects as inspiration only; renderer surfaces consume shared dataset and zone-count artifacts without copying code, adding package dependencies, or hardcoding validation inputs. |
