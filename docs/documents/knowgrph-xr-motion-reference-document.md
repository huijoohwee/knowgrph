...
4. Open first-class FloatingPanel → **Motion Control**, optionally enable the default-off **Bounding box**, keep one full body centered, and explicitly start browser-local pose capture. The box mirrors the existing tracked pose ROI rather than running another detector, and enabling or disabling it never starts the camera. The panel reports permission, backend, confidence, FPS, and latency while raw frames and box coordinates remain ephemeral, and its compact **3D for XR** and **Animation** cards route the same FloatingPanel to those canonical owners.
...
| Motion Control output | 33 normalized landmarks plus 33 hip-relative world landmarks and pose confidence. The default-off preview box reuses the accepted tracked ROI; coordinates are transient and clear with rejected, malformed, low-confidence, or lifecycle-lost tracking instead of remaining frozen. |
| Motion Control control | `knowgrph.control_local_motion_control` accepts `open|start|stop`; `open` alone accepts optional boolean `boundingBox`, and `start` alone accepts `auto|webgpu|wasm`. The exact hydrated forms are `/motion.control @canvas #pose operation=open`, `/motion.control @canvas #pose operation=open boundingBox=true`, `/motion.control @canvas #pose operation=open boundingBox=false`, `/motion.control @canvas #pose operation=stop`, and `/motion.control @canvas #pose operation=start backend=auto|webgpu|wasm`. Toggle controls update the page-session preference without starting the camera; inspection returns only `boundingBoxEnabled` and `boundingBoxAvailable`, not coordinates. WebGPU preference reports pure, partitioned WebGPU+Wasm, or full Wasm execution honestly. |
