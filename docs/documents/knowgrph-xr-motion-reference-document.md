...
| Shared 3D keyboard motion | `threeObjectKeyboardMotion.ts` | One headless utility owns accepted WASD/arrow keys, case normalization, 0.25 m/0.05 m tap steps, frame-speed calculation, diagonal normalization, opposing-key cancellation, and stage-clamped X/Z position resolution for UI and agent callers. |
| Selected-object keyboard motion | `XrObjectKeyboardMotionRuntime.tsx` and `threeObjectInputOwnership.ts` | The XR adapter resolves the selected cast mark, persists shared utility output, and owns browser event/camera isolation only. Press-and-hold movement runs at 2 m/s or Shift 0.4 m/s after a 160 ms intent delay and caps stalled frames at 50 ms. Editable controls and ordinary toolbar/menu controls ignore the binding. The shared object-input owner synchronously pauses OrbitControls and framing writes for the whole key chord, preserves the exact camera pose, and restores camera navigation on final key release. |
...
Given an active graph, when the operator opens Media → 3D, chooses an environment kit, places and labels mobile and static subjects, links SHOOT through the Scene bar or a 3D Object lane/bar, adds camera/cast marks, nudges the selected cast mark with WASD/arrows and the Shift fine step or invokes the equivalent `/animation.control #action-path @selected-actor`/WebMCP move, applies a target-bound Camera move, moves the canonical Timeline playhead, and exports, then XR Mode shows the selected grey-box stage and procedural choreography; human and agent keyboard movement resolve identical bounded X/Z output while the shared camera pose remains invariant; camera samples remain aimed at the linked target; graph metadata contains one normalized plan; and the downloaded package contains the eight virtual files with exact inclusive frame count `floor(duration × fps) + 1`.
