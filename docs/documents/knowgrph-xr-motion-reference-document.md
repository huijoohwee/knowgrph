...
| Selected-object keyboard motion | `XrObjectKeyboardMotionRuntime.tsx` and `threeObjectInputOwnership.ts` | WASD and arrow taps move only the selected cast mark on X/Z by 0.25 m; Shift uses 0.05 m. Press-and-hold movement runs on frame time at 2 m/s or Shift 0.4 m/s after a 160 ms intent delay, caps stalled frames at 50 ms, normalizes diagonals, and cancels opposing keys. Editable controls and ordinary toolbar/menu controls ignore the binding. The shared object-input owner synchronously pauses OrbitControls and framing writes for the whole key chord, preserves the exact camera pose, and restores camera navigation on final key release. |
