...
| Toolbar and Same-Row Controls | Scroll horizontally within the active row from shared toolbar row-scroll classes, keep the primary canvas toolbar thumb-reachable inside safe-area bounds on mobile, and keep icon buttons, launch/dropdown menu rows, design controls, settings previews, chips, labels, table tag lists, and data-view actions clipped through one responsive element-row primitive. | Wrapped icon rows, duplicated overflow classes, component-local scroll rules, top-edge-only mobile toolbars |
...
- Editor sections:
  ...
  - Multi-dimensional Table (host-owned Graph Data Table: canvas fast-grid + Record Inspector split, backed by RxDB GraphTableDb over JSON GraphData; opened only via Workspace toolbar “Workspace: Multi-dimensional Table”, never via Document Mode “Multi-dimensional Table Mode” which is canvas-layout only).
