...
- Implementations must:
  ...
  - Deliverables Widget Card output must remain Markdown SSOT: slide decks reuse `---` slide boundaries and the shared native presentation renderer without generated iframe `srcDoc`; financial models reuse Markdown pipe tables and the shared DataView renderer. HTML/`srcDoc` is not a financial-model persistence format. See `docs/documents/knowgrph-rich-media-deliverables-contract.md`.
