...
- Frame source and startup authority: the published opaque share route carries `kgPreview=1`; Live Canvas Hero isolation adds `kgLiveHero=1` when it owns the selected background. The embedded canvas retains the live `kgShare` route until Source Files finishes restoring origin-scoped persisted state, then imports and activates the shared document last, so persisted active-document state cannot replace an explicit share target on apex load or reload.
