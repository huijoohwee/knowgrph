...
- Prompt preset catalog: `agentic-canvas-os/docs/PROMPT-PRESETS.md`, materialized at `/agentic-canvas-os/docs/PROMPT-PRESETS.md`, is the single prompt-text owner for Video Agent, SME Care Agent, and Investment Research Agent. The FloatingPanel selector reads its labels, descriptions, routes, activation modes, and prompt bodies from this source; duplicate ids, missing entries, unknown routes, or invalid prompt grammar fail closed.
- Video Canvas source: `huijoohwee/docs/knowgrph-agentic-video-canvas-demo.md`, materialized through the shared workspace seed and docs-mirror canonicalization owners, owns the graph and video-script binding but no prompt copy. Runtime video activation prefers the existing `/docs/knowgrph-agentic-video-canvas-demo.md` mirror; an initialization-root alias is adopted only when its basename and source bytes match.
