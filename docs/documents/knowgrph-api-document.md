...
- Rich Media Panel consumers must select one active content surface through the shared neutral resolver and reuse the Storyboard Widget chrome, title typography, compact Card read/edit surface, and single vertical Card scroll owner; full-surface consumers set the shared Card `displayLineClamp` policy to `none`, while row-like consumers retain density clamping. APIs must pass document context and edit capability into that shared surface rather than encode renderer-local read-only markdown, editable text, nested overflow, iframe, image, audio, or video shell variants, or expose a Rich Media-only validation callback that actually performs selection or z-order mutation. Across 2D, 3D, XR, Voxel, and Geospatial projection, connected Markdown remains the panel's native text payload; iframe `srcDoc` is reserved for actual inline HTML and must not be synthesized as a Markdown rendering path.
