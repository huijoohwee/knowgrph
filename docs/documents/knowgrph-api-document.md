...
- Frame source and startup authority: generic generated embeds carry `kgPreview=1`; Live Canvas Hero isolation adds `kgLiveHero=1` for an imported or Source Files-selected background. The canonical Physics Playground startup preset instead preserves the supplied direct opaque share route and removes stale preview, editor, and renderer query overrides so the document frontmatter owns initialization. The embedded canvas retains the live `kgShare` route until Source Files finishes restoring origin-scoped persisted state, then imports and activates the shared document last, so persisted active-document state cannot replace an explicit share target on apex load or reload. On apex Home, route ownership keeps the editorial hero mounted independently of the resolved background renderer; the isolated frame owns XR or 3D rendering while the parent suppresses its own renderer, XR HUD, and workspace inset.
