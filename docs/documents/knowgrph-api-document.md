...
- Probe-Tree per-choice semantic gate and proof boundary: every multi-selection option is validated independently. One bare numeric/range/unit/entity label or one option copied from the selected child's committed answer rejects its complete card, even when sibling options are semantic. Accepted choices express a preference, tradeoff, or consequence; number-bearing choices also explain what the quantity means for the active decision. This gate runs at the shared response projector for provider and literal MCP envelopes, while the active Chat provider, endpoint, and model remain the only transport tuple. `live_provider_run_proven` stays false until an explicitly approved provider run proves real bytes, manifest/read-back identity, playback, synchronized audio/subtitles, Canvas projection, and bounded cost. Dev validation does not authorize Prod or Cloudflare deployment.
