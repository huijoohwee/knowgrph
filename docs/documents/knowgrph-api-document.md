...
- Deliverables Widget bridge: Dev/Preview `POST /__knowgrph_mcp_agentic_os_docs_invoke` accepts a same-origin/same-site JSON request bounded to 32 KiB and 12 normalized invocation tokens. It invokes every requested token only through `knowgrph.agentic_canvas_os.docs.invoke` under one eight-second local-stdio deadline, rejects incomplete or duplicate response coverage, returns literal resolution evidence, and exposes no caller-selected MCP tool, path, upstream endpoint, mutation, deployment, Prod, or Cloudflare action. The canonical Widget Card consumes a typed Rich Media `output -> prompt_in` edge on explicit Run, passes bounded MCP evidence plus the connected source to the configured provider, validates one multi-slide Markdown deck and one Markdown pipe-table financial model, then stages and idempotently persists two workflow-owned Rich Media panels in one final graph write without overwriting authored downstream targets. The deck mounts the shared native Markdown presentation renderer without generated iframe `srcDoc`; the financial model mounts the shared Markdown table renderer. Each panel keeps its local Markdown as the connected lineage edge's display authority. See [agenticOsDocsMcpClient.ts](../../canvas/src/features/agent-ready/agenticOsDocsMcpClient.ts), [storyboardWidgetWorkflowRichMediaDeliverablesRun.ts](../../canvas/src/components/StoryboardWidgetCanvas/runtime/storyboardWidgetWorkflowRichMediaDeliverablesRun.ts), and the [Rich Media Deliverables Widget contract](knowgrph-rich-media-deliverables-contract.md).
