...
- Default source: `huijoohwee/docs/knowgrph-agentic-video-canvas-demo.md`, materialized through the shared workspace seed and docs-mirror canonicalization owners. Runtime preset activation prefers the existing `/docs/knowgrph-agentic-video-canvas-demo.md` mirror; an initialization-root alias is adopted only when its basename and source bytes match.
...
- Activation and execution boundary: **New Chat** allocates the session path without mirroring an empty canonical host file. **Load preset** performs zero provider calls. After a credential is entered, **Send** is explicit approval: each overlapping Markdown graph apply awaits its own exact queued request; implicit and explicitly parsed canvas-preset forms normalize to one semantic request identity, and delayed editor sync revalidates the live Explorer document before it may publish Markdown state. The interceptor accepts activation only when the selected preset metadata and runnable text/image/video plan are committed, applies its `@provider.*` token to the shared generation runtime, and hands that exact graph snapshot to the Storyboard **Run all** owner. For a Chat-triggered run, structured starting/running/completed/terminal status replaces the initiating assistant bubble instead of creating the global Run-all progress toast. The same visible message identities adopt each bounded graph-derived history-key transition, so activation, parsing, and settled-canvas revisions cannot clear the initiating thread or disconnect later progress. The path never enters generic generated-KGC finalization. Historical failed artifacts remain read-only evidence.
...
- Video-frame extraction: Dev/Preview `/__video_frame` writes generated frame images under `/Users/huijoohwee/Documents/GitHub/huijoohwee/image/video-frame/YYYYMMDDTHHmmssZ`, matching the timestamp-token shape used by `/docs_/YYYYMMDDTHHmmssZ` Import URL artifacts.
