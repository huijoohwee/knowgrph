...
API and MCP contracts are owned upstream in Dev. Production mirrors should receive synced artifacts only after upstream validation passes; do not patch generated API behavior inside the publish directory. The Dev-only Agentic ECS surface exposes exactly `knowgrph.ecs.session_start`, `knowgrph.ecs.world_tick`, and `knowgrph.ecs.decision_persist` through the existing local stdio server; it adds no HTTP route, browser session registry, model client, network capability, or deployment lane. Canonical details: [Agentic ECS PRD/TAD](knowgrph-agentic-entity-component-system-prd-tad.md) and [MCP README](../../mcp/README.md).
