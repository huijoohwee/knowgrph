...
- Browser-local WebMCP: `knowgrph.read_local_runtime_identity` returns the application-global canonical identity plus automatic gate snapshot without refreshing, copying, or mutating either owner. Motion Control schema `knowgrph-motion-control-mcp/v2` keeps exactly the existing `knowgrph.inspect_local_motion_control` and `knowgrph.control_local_motion_control` pair. Inspection returns local camera/inference telemetry, canonical XR targets, opaque provider-neutral source/session records, declared dimensions/FPS, calibration and clock-alignment status, synchronized-source/skew/jitter/drop/missing evidence, honest capability tier and `researchReady`, bounded recording counts, deterministic-export readiness, and peer-sharing state; it never returns frames, tensors, landmark arrays, box coordinates, stable device/peer identities, invitations, endpoints, reconstruction evidence content, or export bytes. Structured control and the exact `/motion.control @canvas #pose` grammar converge on `operation=open` with optional `boundingBox=<true|false>`, `operation=start backend=<auto|webgpu|wasm>`, `operation=stop`, `operation=record`, `operation=finish`, `operation=clear`, `operation=export format=<json|csv>`, or `operation=share enabled=<true|false>`. Each optional field is valid only on its named operation; wrong types, casing, duplicate/conflicting fields, mixed input styles, and unknown keys fail closed. Recording is explicit, bounded, browser-local, and derived-landmark-only; Finish or Stop retains a frozen recording, Clear releases it and fences pending export work, Media performs an operator-initiated local download, and WebMCP Export returns digest/count/research-group metadata only. Share is separately opt-in, sends only strict bounded derived observations over the current WebRTC collaboration session, and cannot create an endpoint or invitation. Motion Control, Skills & Commands, and Media project the same runtime while XR owns the Canvas; canonical target actions continue to delegate to `knowgrph.control_local_xr_scene` and `knowgrph.control_local_animation`, and no parallel store, MCP tool, invocation dictionary, scene, renderer, persistence path, or arbitrary model/source URL is added. A monocular or uncalibrated peer source remains `single-view-control`; research-ready requires at least two synchronized metric sources with canonical session-clock or measured source-local alignment, measured calibration provenance, monotonic sequence evidence, explicit shared-session reconstruction evidence, and bounded quality. The complete provider/session contract is documented in `knowgrph-motion-capture-platform-api.md`. Agentic ECS remains a disconnected three-tool local-stdio world/decision lane with no browser WebMCP, scene-rendering, animation-rendering, or physics ownership; caller-held ECS projection may compose only through the existing KGC/Canvas apply seam.
...
- Flight Sim browser-local WebMCP: `knowgrph.inspect_local_flight_sim` projects the active XR Mode flight mission, deterministic aircraft/waypoint state, normalized desktop/touch/gamepad/Motion Control capability, synchronous WebGL gate, zero-call cost ledger, HUD/runtime errors, and browser-local Decision status/path. `knowgrph.control_local_flight_sim` accepts exactly one structured operation or `/flight.sim @canvas #flight operation=<open|start|stop|restart|throttle|save|exit>`; throttle requires one bounded value, while duplicate sigils, mixed input styles, unknown keys, invalid lifecycle operations, and non-terminal saves fail closed. Flight retains the Physics-authored XR terrain, collider catalog, and React Three Fiber Canvas, supplies a pure aircraft follow/framing descriptor to `useXrNativeControllerDemoCamera`, and selects Fixed Follow / Free Orbit through the existing Camera catalog and `/camera.select @camera #camera`. The shared Physics controller hook alone mutates the camera and OrbitControls, while Timeline playback remains the temporary higher-priority framing owner. The Flight tools add no camera driver, renderer, second rendered XR world or scene owner, remote request, model call, published mutation route, or deployment authority.
