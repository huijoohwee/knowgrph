...
- Prompt preset catalog: `agentic-canvas-os/docs/PROMPT-PRESETS.md`, materialized at `/agentic-canvas-os/docs/PROMPT-PRESETS.md`, is the single prompt-text owner for Video Agent, Image to Three.js, Image to GLB, Knowgrph Probe-Tree, SME Care Agent, Investment Research Agent, and Crawler Agent. The FloatingPanel selector reads labels, descriptions, routes, activation modes, and prompt bodies from this source; duplicate ids, missing entries, unknown routes, or invalid prompt grammar fail closed. The Probe-Tree preset inserts only `/knowgrph.probe-tree`; `@knowgrph.probe-tree` and `#knowgrph.probe-tree` remain independently authorable aliases. Selecting a token chip is metadata-only. The bubble-toolbar **Probe-Tree** action remains a deterministic local preview/fallback. Widget **Run** instead sends the selected card's authored context and bounded `/`, `@`, and `#` invocation tokens through the Dev-only Probe-Tree MCP bridge. Provider refinement is card-local and fail-closed: only `probeTreeProviderRefinementApproved: true` permits the configured provider call. The Run path rejects generic or context-mismatched output and commits the accepted cards, inferred edges, and Rich Media ledger through one shared active-graph projector.
