...
- Browser-local WebMCP: `knowgrph.read_local_runtime_identity` returns the application-global canonical identity plus automatic gate snapshot without refreshing, copying, or mutating either owner. `knowgrph.inspect_local_motion_control` returns support, phase, permission, requested/effective backend, fallback reason, model identity, FPS, latency, confidence, and a bounded pose summary; `knowgrph.control_local_motion_control` accepts strict structured control or the exact invocation forms `/motion.control @canvas #pose operation=open`, `/motion.control @canvas #pose operation=stop`, and `/motion.control @canvas #pose operation=start backend=<auto|webgpu|wasm>`, with `backend` valid only for `start`, and routes both input styles through the same active-browser runtime. The Motion Control pair is registered only by app WebMCP because local stdio and published HTTP hosts cannot truthfully reach the active browser camera; inspection never returns images, raw tensors, or pose history, and control cannot upload, record, publish, deploy, or select arbitrary asset URLs.
