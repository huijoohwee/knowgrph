...
- Browser-local WebMCP: `knowgrph.read_local_runtime_identity` returns the application-global canonical identity plus automatic gate snapshot without refreshing, copying, or mutating either owner. `knowgrph.inspect_local_motion_control` returns support, phase, permission, requested/effective backend, fallback reason, model identity, FPS, latency, confidence, a bounded pose summary, selected-humanoid compatibility, compact 3D for XR/Animation target summaries, and preview `boundingBoxEnabled`/`boundingBoxAvailable` booleans; it never exposes box coordinates. The existing `knowgrph.control_local_motion_control` accepts strict structured control or the exact invocation forms `/motion.control @canvas #pose operation=open`, `/motion.control @canvas #pose operation=open boundingBox=<true|false>`, `/motion.control @canvas #pose operation=stop`, and `/motion.control @canvas #pose operation=start backend=<auto|webgpu|wasm>`, and routes both input styles through the same active-browser runtime. `boundingBox` is an optional boolean valid only for `open`; `backend` is valid only for `start`; wrong types, casing, mixed input styles, and unknown keys fail closed. The Bounding box preference is disabled by default, survives Stop/restart for the page session, and reuses the tracked pose ROI; toggling it opens/confirms XR but never starts the camera. Open and Start fail before camera access when XR activation is rejected. The compact target cards reuse `knowgrph.control_local_xr_scene`, `knowgrph.control_local_animation`, and their existing `/`, `@`, and `#` invocations; they add no parallel MCP tool, operation, grammar owner, or arbitrary target input. The Motion Control pair is registered only by app WebMCP because local stdio and published HTTP hosts cannot truthfully reach the active browser camera; inspection never returns images, raw tensors, landmark arrays, box coordinates, or pose history, and control cannot upload, record, publish, deploy, or select arbitrary asset URLs.
