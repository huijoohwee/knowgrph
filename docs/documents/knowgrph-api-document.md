...
- Prompt preset catalog: `agentic-canvas-os/docs/PROMPT-PRESETS.md`, materialized at `/agentic-canvas-os/docs/PROMPT-PRESETS.md`, is the single prompt-text owner for Video Agent, SME Care Agent, Investment Research Agent, and PMF Agent. The FloatingPanel selector reads its labels, descriptions, routes, activation modes, and prompt bodies from this source; empty catalogs, duplicate ids or routes, mismatched ids and routes, unknown routes, or invalid prompt grammar fail closed.
