...
- Purpose: keep localhost Skills & Commands and shared composer menus projected from the sibling `agentic-canvas-os/docs` command, semantic, and binding dictionaries after static downstream invocation arrays were removed. Source Files bootstrap uses the canonical GitHub tree at `huijoohwee/agentic-canvas-os/tree/main/docs`; Cloudflare D1 is a reconciled runtime projection under `agentic-canvas-os/docs/**`, never the authoring owner.
