...
- The shared Rich Media rail exposes the active tab's semantic port at the panel midpoint; text panels therefore expose `output`, not an unrelated media URL port.
...
- active text Rich Media panels exposing the authorable midpoint `output` rail while image panels expose `imageUrl`;
