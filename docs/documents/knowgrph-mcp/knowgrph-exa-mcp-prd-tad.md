...
| `web_search_advanced_exa` is optional and enabled through the `tools` query parameter. | Exa docs | Advanced search must be a deliberate setting or all-non-deprecated profile, not a hidden default. |
| Current non-deprecated tools for this integration are `web_search_exa`, `web_fetch_exa`, and `web_search_advanced_exa`. | Exa docs + repo README | MainPanel should avoid deprecated aliases and list replacements instead of preserving legacy tool names. |
...
- forbid deprecated Exa tool aliases and compatibility remaps
...
| PRD-EXA-MCP-05 | As an auditor, I can see deprecated tool names are not reused. | Given Exa docs list deprecated aliases, when MainPanel MCP renders Exa rows, then deprecated tools are shown only as replacement notes and are not added as enabled tools or compatibility aliases. | Should |
| PRD-EXA-MCP-06 | As an operator, I can opt into advanced search. | Given advanced search is needed, when all non-deprecated tools are enabled, then the URL uses `tools=web_search_exa,web_fetch_exa,web_search_advanced_exa`. | Should |
...
| AC-03 | Advanced search is enabled | The all-tools profile is selected | URL uses exactly the three non-deprecated Exa MCP tools | `/goal all-non-deprecated Exa MCP URL is generated from one constants owner and tools/list smoke returns the expected three tools` |
...
| AC-05 | Deprecated tool names exist upstream | MainPanel MCP renders Exa docs rows | Deprecated names are replacement notes only | `/goal deprecated Exa MCP tool names are absent from enabled tools and config JSON, verified by targeted assertions` |
...
| Deprecated Exa tool aliases in enabled config | 0 desired | 0 | P0 |
...
| Must | Forbid deprecated Exa tool aliases in enabled config | Prevents legacy compatibility drift before implementation. |
| Should | Offer an all-non-deprecated tools profile with `web_search_advanced_exa` | Useful for commerce/company/news filtering, still low cost. |
...
| Won't | Reintroduce deprecated Exa tool names as compatibility aliases | Explicitly forbidden. |
...
5. Focused tests for generated config, forbidden secrets, deprecated tool absence, and MainPanel pipeline readiness.
...
- Adding deprecated Exa MCP tool aliases.
...
| Field | Value |
|---|---|
| Responsibility | Own Exa MCP URLs, server key, tool names, tool profiles, auth labels, deprecated replacements, and default limits. |
...
EXA_MCP_ALL_NON_DEPRECATED_TOOL_NAMES = ['web_search_exa', 'web_fetch_exa', 'web_search_advanced_exa']
...
| Field | Value |
|---|---|
| Responsibility | Render Exa MCP setup, tools, auth, config JSON, deprecated tool replacements, and troubleshooting under MainPanel MCP. |
...
| `tool_profile` | `default` or `all_non_deprecated`. |
| `enabled_tools` | Visible non-deprecated enabled tools. |
...
| `deprecated_replacements` | Deprecated Exa tool names mapped to replacements; not enabled. |
...
| Field | Value |
|---|---|
| Responsibility | Prevent config drift, secret leakage, deprecated tool reintroduction, and direct graph mutation. |
...
| Deprecated aliases | Deprecated tool names are not enabled, mapped, or used as compatibility aliases. | Forbidden token assertions in config generators. |
...
### ADR-003: Forbid Deprecated Exa Tool Aliases
...
Exa docs list deprecated tools and replacement tools. The user explicitly requires cleanup and no legacy/backfill/remapping behavior.

#### Decision

Only enable `web_search_exa`, `web_fetch_exa`, and optionally `web_search_advanced_exa`. Deprecated names can appear only in documentation notes that point to replacements.

#### Alternatives Considered

1. Strict non-deprecated tools only: clean and aligned with current docs.
2. Compatibility aliases: easier for older prompts but creates stale contracts.
...
| Infra cost | 0 | 0 | 0 |
| Egress cost | 0 | 0 | 0 |
| Token cost | Lower due fewer aliases | Higher if prompts include stale aliases | Lower |
| Vendor risk | Lower | Higher stale behavior risk | Lower |

#### Consequences

- Positive: no legacy alias debt.
- Negative: older prompt snippets may need update.
- Neutral: docs can list replacements without enabling old names.
...
| PRD-EXA-MCP-05 | Deprecated replacement rows | `exaMcpApiDocs.ts` | Forbidden enabled-tool assertions |
...
| Deprecated aliases absent from config | Focused unit test | Deprecated names not in enabled tools or generated config. |
...
- Do not add backward-compatible aliases for deprecated Exa tools.
...
| `web_search_advanced_exa` | Optional | Enabled only in all-non-deprecated or advanced profile | Use for category/domain/date/highlight/summary/subpage controls. |
| `get_code_context_exa` | Deprecated | Do not enable | Replacement: `web_search_exa`. |
| `company_research_exa` | Deprecated | Do not enable | Replacement: `web_search_advanced_exa`. |
| `crawling_exa` | Deprecated | Do not enable | Replacement: `web_fetch_exa`. |
| `people_search_exa` | Deprecated | Do not enable | Replacement: `web_search_advanced_exa`. |
| `linkedin_search_exa` | Deprecated | Do not enable | Replacement: `web_search_advanced_exa`. |
| `deep_search_exa` | Deprecated | Do not enable | Replacement: `web_search_advanced_exa`. |
| `deep_researcher_start` | Deprecated in MCP setup page | Do not enable in MainPanel MCP | Route future research API use through a separate contract if needed. |
| `deep_researcher_check` | Deprecated in MCP setup page | Do not enable in MainPanel MCP | Route future research API use through a separate contract if needed. |
