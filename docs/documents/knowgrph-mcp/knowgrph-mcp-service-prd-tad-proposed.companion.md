...
| Browser WebMCP | Shipped | `canvas/src/features/agent-ready/webMcpRuntime.ts` | app runtime registers eight read-only tools, including `knowgrph.inspect_local_workspace_document`, `knowgrph.inspect_local_canvas_topology`, and `knowgrph.inspect_local_canvas_snapshot` |
