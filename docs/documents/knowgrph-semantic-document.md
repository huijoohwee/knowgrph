...
**Algorithm**: Build undirected graph from `semanticRelation` + `coOccursWith` edges, run PageRank-like algorithm for centrality, run label propagation on `coOccursWith` graph for communities, augment entity properties with metrics.
...
graph_metrics_config:
  ...
  label_propagation_iterations:
    type: int
    validation: x ≥ 1
    impact: community detection convergence
```

**Interface Pattern**: `_compute_centrality_communities(entities, semantic_edges, cooccurrence_edges, sem_defaults)` → graph construction → PageRank → label propagation → property augmentation → O(n·e) where n=entities, e=edges
...
| Label Propagation| Detect communities         | - [ ] Propagate labels by majority; iterate; forbid deterministic tie-breaking            | semantic_processor| `_label_propagation`| cooccurrence_graph| community_ids dict | neighbor label voting            |
...
| Graph Metrics   | `knowgrph_parser/semantic_processor.py`| SemanticProcessor| `_compute_centrality_communities`| Analyzer → computes → metrics → augments entities with centrality/communities| `_compute_pagerank`, `_label_propagation`| Adds properties: mentionCount, blockFrequency, centrality, communityId| ~60    |
...
| Centrality/Community| Semantic graph         | Centrality scores, community IDs| `semantic_processor` runs PageRank, label propagation      | O(n·e·i) where n = entities, e = edges, i = iterations|
...
| Label propagation for communities| Lightweight, no hyperparameters| Fast, deterministic for small graphs                  | Non-deterministic tie-breaking at scale   | Use stable sorting or seeded randomness       |
