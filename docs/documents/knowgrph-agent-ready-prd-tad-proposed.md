...
| Browser WebMCP tool registration | Implemented | `canvas/src/features/agent-ready/webMcpRuntime.ts` + `canvas/src/features/agent-ready/knowgrphAgentReadyToolContract.mjs` | App runtime installs ten read-only tools, including browser-local workspace, canvas, and 3d inspectors, and attempts `provideContext`, `registerTool(tool, { signal })`, then readable fallback storage |
