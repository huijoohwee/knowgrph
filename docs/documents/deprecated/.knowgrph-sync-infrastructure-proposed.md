    ...
    subgraph Dev["Dev: knowgrph/"]
        subgraph canvas["canvas/src/"]
            ...
            subgraph fs["Workspace FS layer"]
                fsRxdb["workspaceFsRxdb.ts<br/>createWorkspaceRxdbFs"]
            ...
            subgraph rxdb["RxDB local-first (IndexedDB)"]
                storageRxdb["knowgrphStorageRxdb.ts<br/>documents, chunks, snapshots,<br/>outbox, cursor, devices"]
                recovery["rxdbRecovery.ts<br/>clearRxdbLocalstorageForDatabaseName"]
            end
...
| RxDB | Storage collections | `knowgrphStorageRxdb.ts` | Built |
| RxDB | Recovery | `rxdbRecovery.ts` | Built |
...
flowchart TB
    ...
    subgraph Browser["Browser (any device)"]
        subgraph bfs["Workspace FS layer"]
            bfsRxdb["workspaceFsRxdb (createWorkspaceRxdbFs)"]
