...
**Given** an LLM or connected `knowgrph` MCP host handles a Probe-Tree invocation, **When** it returns `probe-tree-llm-response/v3`, **Then** `response.structuredContent` contains exactly one source Widget anchor, 2–4 concrete Type 2 cards, and one Rich Media branch ledger; each card carries `parentNodeId`, `candidateOptionId`, a model-generated `question` rendered as Summary, 2–4 numbered suggested clarification answers in `selectionOptions`, 2–6 verbatim `contextAnchors` copied from the selected child input or its bounded probe lineage, `selectionMode: multiple`, `allowOther: true`, an empty user-owned `output`, `rationale`, `evidenceNeeded`, `confidence`, bounded `probeTreeDepth`, and `nextAction`. Bare word-fragment splits of a focus phrase fail the shared relevance gate, and the projector infers each `candidateOption` edge from `parentNodeId`.
...
Provider refinement is fail-closed and card-local. Missing, false, or malformed `probeTreeProviderRefinementApproved` values make zero generic provider calls and publish the accepted MCP response or bounded local result directly. For a continuation, canonical Output leads the next context and concrete Output entities/topics outrank generic request verbs, workflow roles, root aliases, and evidence mechanics; bounded ancestor text may shape suggestions only as lineage. Both model and no-model responses must derive each question and every numbered suggestion from that selected-child context and prove the relationship with 2–6 verbatim anchors. Splitting the selected phrase into bare fragments is invalid; the local-zero path instead proposes bounded scope, priority, and constraint directions around each selected focus. Stock evidence, policy, reviewer, approval, or system-of-record choices are rejected unless the user named those concepts. The no-model path enters the Probe-Tree projector as typed input-derived cards without re-parsing generic chat/YAML output; recalled exemplars may guide structure but never supply response content. Global chat configuration alone is not approval.
...
- [x] AI/LLM and MCP response projection proof: `probe-tree-llm-response/v3` requires one source Widget, 2–4 concrete Type 2 next questions with different selected-child focuses, non-overlapping sets of 2–4 suggested clarification answers plus Other, 2–6 verbatim child-or-lineage anchors per card, and one Rich Media branch ledger; repeated questions, bare focus fragments, reused choice labels, and reused/subset/superset choice sets fail closed, multi-selection commits one user-owned canonical Output, and the projector infers each `candidateOption` edge from `parentNodeId`
...
The AI/LLM response path is source-backed at the shared response-projector layer: `/knowgrph.probe-tree`, `#knowgrph.probe-tree`, and `@knowgrph.probe-tree` prompt for `probe-tree-llm-response/v3` under `response.structuredContent`. Each accepted card carries a model-generated question with a different selected-child focus, 2–4 concise suggested clarification answers, 2–6 verbatim anchors copied from the selected child or bounded ancestor lineage, `selectionMode: multiple`, `allowOther: true`, empty user-owned Output, evidence metadata, bounded depth, next action, and `parentNodeId`. The Storyboard Type 2 surface renders numbered checkboxes plus Other and serializes all selections into the canonical Output used by the selected child's next Run. The selected child replaces any same-ID stale graph alias before materialization; the thread root remains lineage metadata only. The shared relevance validator rejects stock content and bare focus fragments in provider, MCP, and no-model paths, while the batch diversity validator rejects repeated questions, reused choice labels, and reused/subset/superset choice sets. The input-derived continuation path creates one distinct focus from each selected authored phrase and proposes bounded scope, priority, and constraint directions instead of slicing a phrase into answer fragments or loading response fixtures.

The 2026-07-16 Canvas enhancement adds the same contract to the centralized Prompt Presets library and the native Widget Run boundary. Selecting the preset loads the canonical `/`, `@`, and `#` trio without submitting. Running the Widget sends the selected context and bounded invocation metadata through the Dev-only same-origin bridge to the actual local stdio server, resolves the three sigils through the read-only Agentic Canvas OS docs tool, and calls exactly `knowgrph.probe.generate`. The configured text provider may refine that literal result once, but only a bounded, context-relevant contract is accepted; unrelated output falls back to the MCP result, while an unavailable or irrelevant MCP result falls back to user-input-derived local-zero options. One shared projector replaces stale candidates and atomically publishes the accepted editable descendants, inferred edges, Mermaid metadata, and owned Rich Media ledger. The bubble-toolbar action remains the deterministic local preview/fallback; it is no longer described as the Widget Run model path.
