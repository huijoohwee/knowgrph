...
If the selected child's Run graph lags the canonical live graph, publication reconciles only exact outgoing user-authored edges from that selected child before resolving targets. It never treats root aliases, panel ownership metadata, candidate lineage, or generated workflow edges as permission to connect or merge output.
