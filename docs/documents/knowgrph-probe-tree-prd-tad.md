...
**Given** an LLM or connected `knowgrph` MCP host handles a Probe-Tree invocation, **When** it returns `probe-tree-llm-response/v3`, **Then** `response.structuredContent` contains exactly one source Widget anchor, 2–4 concrete Type 2 cards, and one Rich Media branch ledger; each card carries `parentNodeId`, `candidateOptionId`, a model-generated `question` rendered as Summary, 2–4 numbered `selectionOptions`, 2–6 verbatim `contextAnchors` copied from the selected user input, `selectionMode: multiple`, `allowOther: true`, an empty user-owned `output`, `rationale`, `evidenceNeeded`, `confidence`, bounded `probeTreeDepth`, and `nextAction`, and the shared projector materializes each result while inferring its `candidateOption` edge from `parentNodeId`.
...
- [x] AI/LLM and MCP response projection proof: `probe-tree-llm-response/v3` requires one source Widget, 2–4 concrete Type 2 next questions with different user-named focuses, non-overlapping sets of 2–4 numbered choices plus Other, 2–6 verbatim selected-input anchors per card, and one Rich Media branch ledger; repeated questions, reused choice labels, and reused/subset/superset choice sets fail closed, multi-selection commits one user-owned canonical Output, and the projector infers each `candidateOption` edge from `parentNodeId`
...
The AI/LLM response path is source-backed at the shared response-projector layer: `/knowgrph.probe-tree`, `#knowgrph.probe-tree`, and `@knowgrph.probe-tree` prompt for `probe-tree-llm-response/v3` under `response.structuredContent`. Each accepted card carries a model-generated question with a different user-named focus, 2–4 concise answer choices, 2–6 verbatim anchors copied from the selected user input, `selectionMode: multiple`, `allowOther: true`, empty user-owned Output, evidence metadata, bounded depth, next action, and `parentNodeId`. The Storyboard Type 2 surface renders numbered checkboxes plus Other and serializes all selections into the canonical Output used by the selected child's next Run. The shared relevance validator rejects stock content in provider, MCP, and no-model paths, while the batch diversity validator rejects repeated questions, reused choice labels, and reused/subset/superset choice sets; the input-derived continuation path creates one distinct focus from each selected authored phrase rather than slicing one phrase list into repetitive windows or loading response fixtures.
