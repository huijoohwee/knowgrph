...
The selected child and its committed Output are the continuation authority. The root and bounded ancestors provide lineage only; a same-ID root alias cannot replace the selected child. Provider transport reads the provider, endpoint, and model only from active Chat runtime state, never from stale card-local routing, fails closed when that route cannot produce at least two accepted cards, and never fills rejected output with hardcoded or zero-model cards. The Probe-specific transport reserves at least 2,400 output tokens with minimal reasoning; an incomplete Responses payload is a transport error even when it contains partial text.
...
The 2026-07-17 transport hardening makes Widget Run itself the provider approval boundary. The bounded MCP envelope remains context only when its cost log reports model `none`; it cannot become a fallback card batch. The provider call uses the active Chat provider, endpoint, and selected model even when the source card contains a contradictory persisted tuple, and a missing active route does not reactivate those stale values. Query-specific card prompts receive the larger output budget needed to finish the typed JSON envelope, while terminal deliverable generation preserves the user's ordinary provider settings. Mixed multi-selection Output remains a continuation unless its authored **Other** value is an imperative deliverable request, in which case only that exact command reaches terminal generation.
