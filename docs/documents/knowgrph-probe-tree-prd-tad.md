...
- The Widget Card bubble-toolbar **Probe-Tree** action immediately runs the deterministic local preview/fallback against the active Storyboard graph. It does not rewrite authored Prompt, Summary, or Action text and does not call MCP or a provider.
...
- Widget Card Run resolves the card's `/`, `@`, and `#` metadata and authored context, invokes local stdio `knowgrph.probe.generate` through one shared 20-second Dev-bridge deadline, and asks the configured provider to refine the literal MCP result only when that card explicitly sets `probeTreeProviderRefinementApproved: true`. Representative invocation evidence resolves in parallel under a shorter sub-budget so it cannot serialize three full timeouts ahead of generation. Generic process cards and context-mismatched provider output are rejected.
...
**Given** an LLM or connected `knowgrph` MCP host handles a Probe-Tree invocation, **When** it returns `probe-tree-llm-response/v1`, **Then** `response.structuredContent` contains exactly one source Widget anchor, 2–4 concrete cards, and one Rich Media branch ledger; each card carries `parentNodeId`, `candidateOptionId`, a model-generated `question` rendered as Summary, an empty editable `output` reserved for the user answer, `rationale`, `evidenceNeeded`, `confidence`, bounded `probeTreeDepth`, and `nextAction`, and the shared projector materializes each result as a `TextGeneration` Probe-Tree Card while inferring its `candidateOption` edge from `parentNodeId`.

> **VCC translation**: `Verify generic Clarify probe / Generate branches / Select handoff cards never appear as LLM contract output, no duplicate explicit edge list is required, Summary owns the generated question, and Output starts empty and accepts the user answer`

**Given** a projected Storyboard card, **When** it is a regular Widget Card (Type 0), **Then** its right pane remains the shared media drop slot; **When** it is a Probe-Tree Card (Type 1), **Then** that media slot is replaced by a full-height **Add output** Viewer pane while Summary remains visible in the left pane. The Output pane reuses `CardInlineTextEditor`, commits through the canonical Output field owner, and contributes the user answer to the bounded MCP/provider context on Run.

> **VCC translation**: `Verify Type 0 keeps media drop, Type 1 replaces media drop with Add output, Summary stays visible during Output edit, exactly one Output contenteditable exists, and Run context contains the committed canonical Output`

**Given** an answered Probe-Tree Card below depth 8, **When** the user presses **Run**, **Then** the committed Output leads the continuation context, the current card id, original `probeTreeThreadRootId`, and incremented `probeTreeDepth` are sent through the same bounded Dev bridge with `recall_top_k: 0`; accepted descendants use the answered card as `parentNodeId`. Existing cards without root/depth metadata recover the root from their parent chain and advance from depth 0. At depth 8, Run returns a visible warning and makes zero MCP/provider calls.
...
Provider refinement is fail-closed and card-local. Missing, false, or malformed `probeTreeProviderRefinementApproved` values make zero generic provider calls and publish the accepted MCP response or bounded local fallback directly. For a continuation, canonical Output leads the next context and concrete Output entities/topics outrank generic request verbs, workflow roles, and evidence mechanics. Context-derived fallback options enter the Probe-Tree projector as typed cards without re-parsing generic chat/YAML output; global chat configuration alone is not approval.
...
**Given** the FloatingPanel Widgets palette, **When** it renders the available choices, **Then** it shows one canonical **Widget Card Type 0**, one **Probe-Tree Type 1**, one consolidated **Rich Media Panel**, and genuinely distinct enabled custom flow editors instead of registry-id text descriptions. Image/Video generation use Rich Media Panel as their single creation surface; provider-specific TextGeneration compatibility aliases and duplicate registry shapes do not create extra Widget Card entries. Every preview follows the active Storyboard **16:9** or **9:16** aspect setting. Type 0 previews the media slot; Type 1 previews **Add output** and seeds the Probe-Tree invocation/card identity when dropped.

> **VCC translation**: `Verify two visual layout items exist, no default/textGeneration description is visible, Type 0 drag keeps the regular Widget Card contract, and Type 1 drag creates a blank Summary plus Output Probe-Tree card`
...
**Given** a Storyboard Widget Card containing `/knowgrph.probe-tree`, `@knowgrph.probe-tree`, or `#knowgrph.probe-tree` in an authored Prompt, Summary, or Action field, including typed frontmatter cells, **When** the user runs the card, **Then** the native Canvas handler invokes bounded local `knowgrph.probe.generate` with the selected card's authored context and `/`, `@`, and `#` invocation metadata before generic prompt handling; it may submit the literal MCP result to the configured provider for one refinement, accepts only relevant `probe-tree-llm-response/v1`, replaces the stale Probe-Tree descendant closure, materializes 2–4 editable `TextGeneration` Probe-Tree Cards with canonical inferred `candidateOption` edges in the balanced grid-snapped waterfall, preserves the current viewport, and atomically publishes their evidence ledger in the owned Rich Media Panel.
...
| Canvas runtime | Widget Card → MCP/provider response → branch cards + Rich Media ledger | `canvas/viteProbeTreeMcpBridge.ts`, `canvas/src/components/StoryboardWidgetCanvas/runtime/storyboardWidgetWorkflowProbeTreeRun.ts`, `canvas/src/components/StoryboardWidgetCanvas/runtime/storyboardWidgetProbeTreeStructuredResponse.ts`, `canvas/src/components/StoryboardWidgetCanvas/runtime/storyboardWidgetProbeTreeLayout.ts` | Implemented — local stdio MCP under one 20-second deadline, parallel bounded invocation-evidence lookup, card-approved relevance-validated provider refinement, collision-free branch bands, one thread-grouped internally scrolling ledger, atomic active-graph projection, depth limit 8 |
...
- [x] AI/LLM and MCP response projection proof: `probe-tree-llm-response/v1` requires one source Widget, 2–4 concrete next questions, at most one material clarification, and one Rich Media branch ledger; `response.structuredContent.cards` projects each result as an editable `TextGeneration` Probe-Tree Card with the generated question in Summary and a blank user-owned Output, and infers its `candidateOption` edge from `parentNodeId` without a duplicate explicit edge list, while generic process cards remain fallback-only
...
- [x] Rich Media output proof: native Widget Run leaves the source card unchanged, calls bounded local MCP before optional provider refinement, rejects unrelated output, and publishes accepted cards plus the branch ledger through the shared terminal graph transaction
...
The AI/LLM response path is source-backed at the shared response-projector layer: `/knowgrph.probe-tree`, `#knowgrph.probe-tree`, and `@knowgrph.probe-tree` prompt for `probe-tree-llm-response/v1` under `response.structuredContent`. The contract asks for one source Widget anchor copied from selected context, 2–4 context-specific cards, and one Rich Media branch ledger; it permits at most one clarification only when a named missing fact changes branch selection and rejects the deterministic Clarify/Generate/Select process cards as model output. Each card carries its model-generated question, empty user-answer Output, rationale, evidence requirement, confidence, bounded depth, next action, and `parentNodeId`; the parser and projector clear any provider-authored Output, render the question as Summary, keep Output editable for the user, and infer the `candidateOption` edge without requiring a duplicate edge payload. The local stdio `knowgrph.probe.generate` tool emits this same nested response, and literal MCP envelopes are recursively unwrapped through the existing Chat -> KGC -> Canvas apply path. The Storyboard/Mermaid bridge remains source-backed instead of illustrative-only.

The 2026-07-16 Canvas enhancement adds the same contract to the centralized Prompt Presets library and the native Widget Run boundary. Selecting the preset loads the canonical `/`, `@`, and `#` trio without submitting. Running the Widget sends the selected context and bounded invocation metadata through the Dev-only same-origin bridge to the actual local stdio server, resolves the three sigils through the read-only Agentic Canvas OS docs tool, and calls exactly `knowgrph.probe.generate`. The configured text provider may refine that literal result once, but only a bounded, context-relevant contract is accepted; unrelated output falls back to the MCP result, while an unavailable or irrelevant MCP result falls back to context-derived local-zero options. One shared projector replaces stale candidates and atomically publishes the accepted editable descendants, inferred edges, Mermaid metadata, and owned Rich Media ledger. The bubble-toolbar action remains the deterministic local preview/fallback; it is no longer described as the Widget Run model path.

Text workflow publication now treats the source Widget output, owned Rich Media Panel, and connecting edge as one terminal graph transaction. Probe-Tree Run stages its materialized branches and Rich Media ledger in that single commit, including when an existing frontmatter-backed panel stores properties in typed `{key,type,value}` envelopes. Streaming updates remain draft-only. If a prior run wrote its workspace output artifact but the document retained a stale loading revision, the Canvas reads that same artifact and completes the terminal transaction locally after the staleness guard; it does not repeat the provider request.
