...
**Given** an LLM or connected `knowgrph` MCP host handles a Probe-Tree invocation, **When** it returns `probe-tree-llm-response/v2`, **Then** `response.structuredContent` contains exactly one source Widget anchor, 2–4 concrete Type 2 cards, and one Rich Media branch ledger; each card carries `parentNodeId`, `candidateOptionId`, a model-generated `question` rendered as Summary, 2–4 numbered `selectionOptions`, `selectionMode: multiple`, `allowOther: true`, an empty user-owned `output`, `rationale`, `evidenceNeeded`, `confidence`, bounded `probeTreeDepth`, and `nextAction`, and the shared projector materializes each result while inferring its `candidateOption` edge from `parentNodeId`.
...
- [x] AI/LLM and MCP response projection proof: `probe-tree-llm-response/v2` requires one source Widget, 2–4 concrete Type 2 next questions with 2–4 numbered choices plus Other, and one Rich Media branch ledger; multi-selection commits one user-owned canonical Output and the projector infers each `candidateOption` edge from `parentNodeId`
...
The AI/LLM response path is source-backed at the shared response-projector layer: `/knowgrph.probe-tree`, `#knowgrph.probe-tree`, and `@knowgrph.probe-tree` prompt for `probe-tree-llm-response/v2` under `response.structuredContent`. Each accepted card carries a model-generated question, 2–4 concise answer choices, `selectionMode: multiple`, `allowOther: true`, empty user-owned Output, evidence metadata, bounded depth, next action, and `parentNodeId`. The Storyboard Type 2 surface renders numbered checkboxes plus Other and serializes all selections into the canonical Output used by the selected child's next Run.
