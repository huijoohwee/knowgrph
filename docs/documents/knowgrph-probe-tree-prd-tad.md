...
- [x] AI/LLM and MCP response projection proof: `probe-tree-llm-response/v3` requires one source Widget, 2–4 concrete Type 2 next questions with 2–4 numbered choices plus Other, 2–6 verbatim selected-input anchors per card, and one Rich Media branch ledger; multi-selection commits one user-owned canonical Output and the projector infers each `candidateOption` edge from `parentNodeId`
...
The AI/LLM response path is source-backed at the shared response-projector layer: `/knowgrph.probe-tree`, `#knowgrph.probe-tree`, and `@knowgrph.probe-tree` prompt for `probe-tree-llm-response/v3` under `response.structuredContent`. Each accepted card carries a model-generated question, 2–4 concise answer choices, 2–6 verbatim anchors copied from the selected user input, `selectionMode: multiple`, `allowOther: true`, empty user-owned Output, evidence metadata, bounded depth, next action, and `parentNodeId`. The Storyboard Type 2 surface renders numbered checkboxes plus Other and serializes all selections into the canonical Output used by the selected child's next Run. The same relevance validator rejects stock content in provider, MCP, and no-model paths; the input-derived path slices authored enumerations rather than loading response fixtures.
