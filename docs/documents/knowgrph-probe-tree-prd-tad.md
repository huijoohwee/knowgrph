...
Provider refinement is fail-closed and card-local. Missing, false, or malformed `probeTreeProviderRefinementApproved` values make zero generic provider calls and publish the accepted MCP response or bounded local fallback directly; global chat configuration alone is not approval.
