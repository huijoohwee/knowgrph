...
- Run `npm run test:live:storyboard-media-panel-retention:browser` for the real app-route SSOT restoration proof: it mounts the normal Canvas route, injects `huijoohwee/docs/knowgrph-strybldr-starter-template.md` through `window.knowgrphWorkspaceCommand.applyMarkdownDocument(...)`, creates one image and one video `Rich Media Panel`, verifies panel box stability through edge creation/retention, reapplies the markdown SSOT, and proves the transient panels and created edges fully disappear back to the baseline live-route state. The contract is explicit: same-path same-text reapply still counts as a fresh source-authority reset, so Flow Editor draft lifetime must invalidate via an applied-document reapply epoch rather than relying on path or text deltas alone.
