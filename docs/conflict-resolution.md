...
- `npm run --silent collaboration:report:example | npm run --silent collaboration:report:check -- --json -` emits machine-readable validation identity; failures retain a nonzero exit code and emit stable error codes instead of a Node stack trace. Automation must use this mode instead of parsing human output.
