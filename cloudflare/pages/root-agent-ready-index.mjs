...
const rewriteRootAppHtml = (html) => {
  ...
  if (/<meta\s+name=["']description["'][^>]*>/i.test(next)) {
    next = next.replace(/<meta\s+name=["']description["'][^>]*>/i, `<meta name="description" content="${ROOT_DESCRIPTION}" />`);
  } else {
...
const loadKnowgrphAppShell = async (request) => {
  ...
  if (!html.includes(ROOT_MOUNT_MARKUP) || !html.includes(`${APP_BASE_PATH}/assets/`)) return null;
...
export async function onRequest(context) {
  ...
  const rootAppHtml = method === "HEAD" ? null : await loadKnowgrphAppShell(request);
  const html = withAgentReadyRouteHeaders(rootAppHtml || rootHtmlResponse(method === "HEAD" ? "" : await loadWebMcpScript(request)), {
    owner: ROOT_AGENT_READY_ROUTE_OWNER,
