...
const devServerPort = String(process.env.KG_RICH_MEDIA_SMOKE_PORT || '4175')
const devServerBaseUrl = `http://localhost:${devServerPort}`
const devServerUrl = `${devServerBaseUrl}/__smoke__/rich-media`
...
async function run() {
  ...
  if (!reuseExistingServer) {
    ...
    console.log(`[rich-media-browser-smoke] reusing existing dev server at ${devServerUrl}`)
  }
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      const smoke = spawn('python3', ['scripts/verify_rich_media_browser_smoke.py'], {
        cwd: process.cwd(),
        stdio: 'inherit',
        env: {
          ...process.env,
          KG_RICH_MEDIA_SMOKE_BASE_URL: devServerBaseUrl,
        },
      })
