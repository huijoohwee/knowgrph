...
import {
  STORYBOARD_WIDGET_LOOP_NODE_TYPE,
  STORYBOARD_WIDGET_NODE_KIND_LOOP_VALUE,
  STORYBOARD_WIDGET_NODE_KIND_PROPERTY_KEY,
} from '@/lib/config'
...
export function isLoopNode(node: Pick<GraphNode, 'type' | 'properties'> | null | undefined): boolean {
  ...
  if (type !== STORYBOARD_WIDGET_LOOP_NODE_TYPE) return false
  const props = (node as { properties?: unknown }).properties
  if (!props || typeof props !== 'object' || Array.isArray(props)) return false
  return (props as Record<string, unknown>)[STORYBOARD_WIDGET_NODE_KIND_PROPERTY_KEY] === STORYBOARD_WIDGET_NODE_KIND_LOOP_VALUE
}
...
export function convertNodeToLoopInGraphData(graphData: GraphData, nodeId: string): { changed: boolean; graphData: GraphData } {
  ...
  const nextProps: Record<string, unknown> = {
    ...prevProps,
    [STORYBOARD_WIDGET_NODE_KIND_PROPERTY_KEY]: STORYBOARD_WIDGET_NODE_KIND_LOOP_VALUE,
  }
  nextNodes[idx] = { ...target, type: STORYBOARD_WIDGET_LOOP_NODE_TYPE, properties: nextProps as never }
