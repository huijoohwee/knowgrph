...
export type StoryboardWidgetPortRowDirection = 'input' | 'output'

export type StoryboardWidgetPortRow = {
  key: string
  nodeId: string
  nodeLabel: string
  nodeType: string
  direction: StoryboardWidgetPortRowDirection
  portKey: string
  socketType: string
...
export type StoryboardWidgetPortRowsSummary = {
  inputCount: number
  outputCount: number
  connectedCount: number
  rows: StoryboardWidgetPortRow[]
}
...
const buildRowsForNodeDirection = (args: {
  ...
  node: GraphNode
  dir: 'in' | 'out'
}): StoryboardWidgetPortRow[] => {
  const portKeys = listTypedFlowPortKeys(args.node, args.dir)
  const direction: StoryboardWidgetPortRowDirection = args.dir === 'in' ? 'input' : 'output'
...
export function buildStoryboardWidgetPortRows(graphData: Pick<GraphData, 'nodes' | 'edges'> | null | undefined): StoryboardWidgetPortRowsSummary {
  const nodes = Array.isArray(graphData?.nodes) ? [...graphData.nodes].sort(compareNodePosition) : []
  const rows: StoryboardWidgetPortRow[] = []
...
export function resolveStoryboardWidgetFocusedEdgeIds(
