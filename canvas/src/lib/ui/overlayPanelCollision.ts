...
export function resolveOverlayPanelCollisions(args: {
  items: Array<{ id: string; top: number; left: number; movable: boolean }>
  panelSize: { width: number; height: number }
  gapPx: number
  strength: number
  iterations: number
}): Array<{ id: string; top: number; left: number }> {
  const w = Number.isFinite(args.panelSize?.width) ? Math.max(0, args.panelSize.width) : 0
  const h = Number.isFinite(args.panelSize?.height) ? Math.max(0, args.panelSize.height) : 0
  if (w <= 0 || h <= 0) return args.items.map(it => ({ id: it.id, top: it.top, left: it.left }))
