...
import { FLOW_WIDGET_KV_KEY_COLUMN_STYLE, FLOW_WIDGET_KV_ROW_LAYOUT, FLOW_WIDGET_KV_VALUE_COLUMN_STYLE } from '@/components/FlowEditor/nodeOverlayEditorTableLayout'
...
type FlowEditorPortRowKeyParts = {
  nodeId: string
  direction: 'in' | 'out'
  portKey: string
}

const parseFlowEditorPortRowKey = (rowKey: string): FlowEditorPortRowKeyParts | null => {
...
export const NodeOverlayEditorKvTable = React.memo(function NodeOverlayEditorKvTable(props: {
  ...
  const selectedFlowEditorPortRowKey = useGraphStore(
    useShallow(state => state.flowEditorSelectedPortRowKey || ''),
  )
  ...
  React.useLayoutEffect(() => {
    const parsed = parseFlowEditorPortRowKey(selectedFlowEditorPortRowKey)
  ...
  }, [rows, selectedFlowEditorPortRowKey])
          ...
          {(rows || []).map(row => {
            ...
            return (
              <tr
                ...
                data-kg-flow-editor-port-row-key={selected ? selectedFlowEditorPortRowKey : undefined}
              >
                <td
                  className={cn(
                    'py-2 align-middle',
