...
const FlowWidgetOverlayInner = React.memo(function FlowWidgetOverlayInner({
  ...
  const effectiveHideFields = isRichMediaPanelNode ? richMediaKtvRows : hideFields
  ...
  const persistFloatingScreenPlacement = placement.persistFloatingScreenPlacement
  const readCurrentOverlayScreenPlacementForHandoff = placement.readCurrentOverlayScreenPlacementForHandoff
  const lastAppliedPlacementRef = placement.lastAppliedRef
  ...
  const setPinnedInCanvas = React.useCallback((next: boolean | ((prev: boolean) => boolean)) => {
    ...
    if (
      ...
      const currentScreenPlacement = readCurrentOverlayScreenPlacementForHandoff()
      const applied = currentScreenPlacement || lastAppliedPlacementRef.current
      if (applied) persistFloatingScreenPlacement({ top: applied.top, left: applied.left })
    }
  ...
  }, [graphMetaKey, graphMetaKind, lastAppliedPlacementRef, node, nodeId, persistCurrentScreenPlacementAsWorldPlacement, persistFloatingScreenPlacement, readCurrentOverlayScreenPlacementForHandoff, setFlowWidgetPinnedByNodeIdForGraph, storyboardWidgetSurfaceId])
  ...
  const {
    ...
    connectedValuesBySchemaPath,
    onPatchProperties,
    onToggleHideFields: handleToggleRichMediaKtvRows,
  })
