...
export function useFlowEditorInspectorSurface(args: {
  active: boolean
  ...
  const createSubgraphFromSelection = React.useCallback((nextArgs: { label?: string; kind?: 'subgraph' | 'cluster' }) => {
    ...
    if (res.ok === false) {
      args.upsertUiToast({ id: 'flow-editor-subgraph-create-failed', kind: 'warning', message: res.message, ttlMs: 2500 })
      return
    }
  ...
  const inspectorElement = (
    <FlowEditorInspector
      ...
      onSetSubgraphKind={(id, kind) => wrapSubgraphMutation(`flow-editor-subgraph-kind-failed-${String(id || '')}`, UI_COPY.flowEditorRunFailedToast, () => args.updateUserSubgraph(id, { kind }))}
      onRenameSubgraph={(id, label) => wrapSubgraphMutation(`flow-editor-subgraph-rename-failed-${String(id || '')}`, UI_COPY.flowEditorRunFailedToast, () => args.updateUserSubgraph(id, { label }))}
      onDeleteSubgraph={args.removeUserSubgraph}
      onSetSubgraphParent={(id, parentId) => wrapSubgraphMutation(`flow-editor-subgraph-parent-failed-${String(id || '')}`, UI_COPY.flowEditorRunFailedToast, () => args.updateUserSubgraph(id, { parentId }))}
      onAddSelectionToSubgraph={(id) => {
        const nodeIds = (args.selectedNodeIds || []).map(v => String(v || '').trim()).filter(Boolean)
        wrapSubgraphMutation(`flow-editor-subgraph-add-failed-${String(id || '')}`, UI_COPY.flowEditorRunFailedToast, () => args.addNodesToUserSubgraph(id, nodeIds))
      }}
      onRemoveSelectionFromSubgraph={(id) => {
        const nodeIds = (args.selectedNodeIds || []).map(v => String(v || '').trim()).filter(Boolean)
        wrapSubgraphMutation(`flow-editor-subgraph-remove-failed-${String(id || '')}`, UI_COPY.flowEditorRunFailedToast, () => args.removeNodesFromUserSubgraph(id, nodeIds))
      }}
