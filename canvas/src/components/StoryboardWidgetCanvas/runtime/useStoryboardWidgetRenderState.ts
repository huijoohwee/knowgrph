...
export function useStoryboardWidgetRenderState(args: {
  ...
  const appliedHistoryRestoreRevisionRef = React.useRef(args.historyRestoreRevision)
  ...
  React.useLayoutEffect(() => {
    ...
    const historyRestoreRevisionChanged = appliedHistoryRestoreRevisionRef.current !== args.historyRestoreRevision
    const nextDraft = resolveStoryboardWidgetDraftGraphDataForBaseReset({
      ...
      forceBaseReset: historyRestoreRevisionChanged,
    })
    appliedHistoryRestoreRevisionRef.current = args.historyRestoreRevision
  ...
  }, [args.activeDocumentKey, args.baseGraphDataRevision, args.editorRuntimeActive, args.historyRestoreRevision, storyboardWidgetBaseContentSignature])
