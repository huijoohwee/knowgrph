...
export function buildStoryboardWidgetProbeTreeProviderPrompt(args: {
  ...
  return [
    ...
    '- Emit exactly 2-4 records in cards. Each record must contain only id, question, rationale, evidenceNeeded, probeTreeCardVariant: probe-tree-type-2, 2-4 concise string selectionOptions, and 2-6 contextAnchors copied verbatim from the selected user input.',
    `- The runtime owns source Widget ${args.currentNodeId}, parentNodeId, candidateOptionId, depth ${args.probeTreeDepth}, selectionMode, allowOther, nextAction, empty user Output, candidate edges, and the Rich Media ledger. Do not emit widgets, panels, edges, those runtime fields, or copied source records.`,
