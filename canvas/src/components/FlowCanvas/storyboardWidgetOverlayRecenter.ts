...
export function recenterStoryboardWidgetOverlayWidgetPositions(args: {
...
const readOverlayNodeIdsForSurface = (activeSurfaceId: string): Set<string> => {
  ...
  const selectors = [STORYBOARD_WIDGET_OVERLAY_ROOT_SELECTOR, RICH_MEDIA_OVERLAY_ROOT_SELECTOR]
  for (let i = 0; i < selectors.length; i += 1) {
    const selector = selectors[i]!
    const roots = Array.from(document.querySelectorAll(selector)).filter(
      (el): el is HTMLElement =>
        el instanceof HTMLElement
        && readStoryboardWidgetOverlaySurfaceId(el) === activeSurfaceId,
    )
