...
import {
  __resetGraphRecordDbForTests,
  allocateNewRowId,
  createRowFromGraphEntity,
  ensureGraphRecordSeed,
  getGraphRecordDb,
  syncGraphDataToGraphRecordDb,
...
const resetGraphRecordDb = async () => {
  await __resetGraphRecordDbForTests()
}

export async function testGraphRecordDbSeedsBaseTablesAndColumns() {
  await resetGraphRecordDb()
...
export async function testGraphRecordDbSyncsGraphAndInfersPropertyColumns() {
  await resetGraphRecordDb()
  ...
  await syncGraphDataToGraphRecordDb(graph)
  const { collections } = await getGraphRecordDb()
...
export async function testGraphRecordDbInfersAndUpgradesDateColumns() {
  await resetGraphRecordDb()
  ...
  await syncGraphDataToGraphRecordDb(graph1)
  const { collections } = await getGraphRecordDb()
  ...
  await syncGraphDataToGraphRecordDb(graph2)
...
export async function testGraphRecordDbConcurrentSyncDoesNotConflict() {
  await resetGraphRecordDb()
  const graph: GraphData = {
    type: 'Graph',
  ...
  await Promise.all([syncGraphDataToGraphRecordDb(graph), syncGraphDataToGraphRecordDb(graph)])
  const { collections } = await getGraphRecordDb()
...
export async function testGraphRecordDbWarmAndSyncDoNotConflict() {
  await resetGraphRecordDb()
  const graph: GraphData = {
    type: 'Graph',
  ...
  await Promise.all([warmGraphRecordDb(), syncGraphDataToGraphRecordDb(graph)])
  const { collections } = await getGraphRecordDb()
...
export async function testGraphRecordDbNoopSyncDoesNotRewriteRows() {
  await resetGraphRecordDb()
  const graph: GraphData = {
    type: 'Graph',
    nodes: [{ id: 'n1', type: 'Person', label: 'Alice', properties: { age: 3 } }],
...
export async function testGraphRecordDbUpdatesCellValues() {
  await resetGraphRecordDb()
  const graph: GraphData = {
    type: 'Graph',
    nodes: [{ id: 'n1', type: 'Person', label: 'Alice', properties: { age: 3 } }],
  ...
  await updateGraphRecordCell('nodes', 'n1', 'age', 4)
  const { collections } = await getGraphRecordDb()
  const row = await collections.rows.findOne('nodes:n1').exec()
...
export async function testGraphRecordDbAllocatesAndCreatesRows() {
  await resetGraphRecordDb()
  await ensureGraphRecordSeed()
...
export async function testGraphRecordDbSyncsCollapsedGraphViewRows() {
  await resetGraphRecordDb()
  const graph: GraphData = {
    type: 'Graph',
    nodes: [
  ...
  await syncGraphDataToGraphRecordDb(collapsed)
