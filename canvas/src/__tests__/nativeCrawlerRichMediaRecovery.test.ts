...
export function testNativeCrawlerRichMediaProgressAndTerminalRecovery() {
  ...
  if (!orphanedBackfillPanel || orphanedBackfillPanel.connectedText || orphanedBackfillPanel.isLoading !== true) throw new Error('crawler Rich Media output must never backfill from an unconnected Widget Card')
}
