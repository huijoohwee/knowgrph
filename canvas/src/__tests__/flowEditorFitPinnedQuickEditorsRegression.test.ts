...
export function testFlowEditorFitIncludesPinnedQuickEditors() {
  const nodes = [
    {
      id: 'n1',
      type: 'Test',
  ...
  const withPinned = fitFlowEditorPinnedQuickEditors({
    nodes: nodes as never,
    fitW,
    viewportH: fitH,
    viewportW: fitW,
    openQuickEditorNodeIds: ['n1'],
    pinnedById: { n1: true },
  ...
  if (!(withPinned.k < base.k - 1e-6)) {
    throw new Error(`expected pinned quick editors to reduce fit scale, base=${base.k} next=${withPinned.k}`)
  }
}
