...
export function testRichMediaPanelUsesSectionBodyResizeHandleSsot() {
  const p = resolve(process.cwd(), 'src', 'components', 'StoryboardWidget', 'WidgetEditorPanel.tsx')
...
export function testSharedRichMediaPanelUsesRootFrameAsResizeSurfaceSsot() {
  ...
  if (
    !typesText.includes("panelChrome?: 'none' | 'storyboardWidget'")
    || !shellText.includes("from '@/components/StoryboardWidget/StoryboardWidgetPanelChrome'")
    || !shellText.includes('data-kg-rich-media-storyboard-widget-body="1"')
  ) {
...
export function testRichMediaPanelStoryboardWidgetChromeMaintainsContentAspectAcrossZoom() {
  const aspect = 16 / 9
  ...
  for (const snippet of [
    ...
    if (!chromeText.includes(snippet)) {
      throw new Error(`expected Storyboard Widget rich-media chrome header to bind layout to shared media-panel sizing variable: ${snippet}`)
    }
  }
...
export function testRichMediaPanelResizeDragMaintainsContentAspectFromSharedMath() {
  ...
  if (Math.abs(widgetAspect - RICH_MEDIA_PANEL_FRAME_ASPECT) > 0.005) {
    throw new Error(`expected Storyboard Widget resize math to normalize stale panel aspect to 16:9, got ${widgetAspect}`)
  }
  ...
  if (Math.abs(staleWidePanelAspect - RICH_MEDIA_PANEL_FRAME_ASPECT) > 0.005) {
    throw new Error(`expected Storyboard Widget load coercion to repair stale non-16:9 saved dimensions, got ${staleWidePanelAspect}`)
  }
  ...
  if (Math.abs(portraitAspect - RICH_MEDIA_PANEL_PORTRAIT_FRAME_ASPECT) > 0.005) {
    throw new Error(`expected Storyboard Widget resize math to preserve 9:16 toolbar-selected aspect, got ${portraitAspect}`)
  }

  const storyboardWidgetHookPath = resolve(process.cwd(), 'src', 'components', 'StoryboardWidget', 'useRichMediaWidgetPreview.ts')
  const storyboardWidgetHookText = readFileSync(storyboardWidgetHookPath, 'utf8')
  if (!storyboardWidgetHookText.includes('computeRichMediaPanelAspectResizeSizePx')) {
    throw new Error('expected Storyboard Widget Rich Media Panel resize to reuse shared aspect-preserving resize math')
  }
  for (const snippet of [
    ...
    if (!storyboardWidgetHookText.includes(snippet)) {
      throw new Error(`expected Storyboard Widget Rich Media Panel resize end to commit latest drag size without stale React state: ${snippet}`)
    }
  }
  if (storyboardWidgetHookText.includes('richMediaPanelResizeStartRef.current.width + args0.dx') || storyboardWidgetHookText.includes('richMediaPanelResizeStartRef.current.height + args0.dy')) {
    throw new Error('expected Storyboard Widget Rich Media Panel resize to avoid independent width/height delta resizing')
  }
  const storyboardWidgetViewPath = resolve(process.cwd(), 'src', 'components', 'StoryboardWidget', 'WidgetEditorView.tsx')
  const storyboardWidgetViewText = readFileSync(storyboardWidgetViewPath, 'utf8')
  if (storyboardWidgetViewText.includes('readRichMediaPanelFrameWidthPx') || storyboardWidgetViewText.includes('richMediaPanelFrameWidthPx')) {
    throw new Error('expected Storyboard Widget Rich Media Panel wrapper to avoid duplicate width ownership outside the panel frame')
  }
  const storyboardWidgetPanelPath = resolve(process.cwd(), 'src', 'components', 'StoryboardWidget', 'WidgetEditorPanel.tsx')
  const storyboardWidgetPanelText = readFileSync(storyboardWidgetPanelPath, 'utf8')
  for (const snippet of [
    ...
    if (!storyboardWidgetPanelText.includes(snippet)) {
      throw new Error(`expected Storyboard Widget Rich Media Panel outer frame to own selected-aspect resize dimensions: ${snippet}`)
    }
  }
  if (storyboardWidgetPanelText.includes('richMediaPreviewPassThrough')) {
    throw new Error('expected Storyboard Widget Rich Media Panel media surface to remain selectable instead of passing pointer events through the body')
  }
  if (storyboardWidgetPanelText.includes("height: `${richMediaPanelViewSize.height}px`")) {
  ...
  for (const snippet of [
    ...
    if (!richMediaPanelText.includes(snippet)) {
      throw new Error(`expected embedded Rich Media Panel surface to remain inside Storyboard Widget panel stacking: ${snippet}`)
    }
  }
  ...
  for (const snippet of [
    ...
    if (!placementRuntimeText.includes(snippet)) {
      throw new Error(`expected Storyboard Widget placement runtime to align Rich Media wrapper geometry with the selected-aspect panel frame: ${snippet}`)
    }
  }
}
...
export function testStoryboardWidgetOverlayProxyTreatsRichMediaResizeHandleAsProtectedHandle() {
  const p = resolve(process.cwd(), 'src', 'lib', 'canvas', 'storyboard-widget-overlay-proxy.ts')
  const text = readFileSync(p, 'utf8')
  if (!text.includes('[data-kg-resize-handle]')) {
    throw new Error('expected Storyboard Widget overlay proxy drag-handle selector to include RichMediaPanel resize handles so window-capture proxy pan cannot steal resize drags')
  }
}
