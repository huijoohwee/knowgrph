...
export function testProbeTreeGenerationDoesNotRearmStoryboardInitialFit() {
  ...
  assert(flowRuntimeSource.includes('const initKey = storyboardWidgetMode ? `storyboardWidget:${zoomViewKey}` : zoomViewKey'), 'expected the native Storyboard Flow surface to fit once per document/view rather than once per topology')
