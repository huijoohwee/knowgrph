...
export function testCanvasSurfaceMode3dSelectionUsesSharedOwner() {
  ...
  if (canvasViewActionsText.includes("setCanvas3dMode('3d')") || !canvasViewActionsText.includes('onOpenShared3dPanel?.(mode)') || !canvasViewActionsText.includes("mode === '3d' ? 'camera' : 'xr'") || !canvasViewActionsText.includes("setBottomSurfaceTab('timeline')") || !canvasViewActionsText.includes('setBottomSurfaceCollapsed(false)')) {
    throw new Error('Expected 3D to open Camera while XR opens its panel plus the canonical BottomPanel Timeline lane')
  }
