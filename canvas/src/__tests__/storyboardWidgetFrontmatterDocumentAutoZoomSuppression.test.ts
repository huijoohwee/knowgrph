...
export function testStoryboardWidgetFrontmatterDocumentSuppressesAutoZoomModes() {
  const p = resolve(process.cwd(), 'src', 'components', 'FlowCanvas.tsx')
  const text = readFileSync(p, 'utf8')
  if (!text.includes("canvas2dRenderer === 'storyboard'")) {
    throw new Error('expected FlowCanvas auto-zoom suppression to target the Storyboard renderer')
  }
  ...
  if (!text.includes('const autoZoomPaused = !active || (suppressAutoZoomModes && !fitToScreenMode && !zoomToSelectionMode)')) {
    throw new Error('expected FlowCanvas to pause auto-zoom only when Storyboard Widget frontmatter document overlays are active and no explicit auto-zoom mode is enabled')
  }
