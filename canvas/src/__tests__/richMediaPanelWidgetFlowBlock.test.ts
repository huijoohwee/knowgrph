...
export function testFrontmatterFlowWidgetsConnectToRichMediaPanelPorts() {
  const md = [
    ...
    '      label: {key: label, type: string, value: "ByteDance-Seedream-4.0 Image Widget"}',
    ...
    '      model: {key: model, type: string, value: "ByteDance-Seedream-4.0"}',
    '      prompt: {key: prompt, type: string, value: "image"}',
    '    - id: {key: id, type: string, value: "w-byteplus-video"}',
    ...
    '      model: {key: model, type: string, value: "ByteDance-Seedance-1.0-pro-fast"}',
    ...
    '    - { id: e3, source: w-byteplus-video, sourceHandle: videoUrl, target: p-rich-media, targetHandle: videoUrl, animated: true }',
    '---',
    '',
    '# Body',
  ].join('\n')
