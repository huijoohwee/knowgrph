...
export function testStoryboardWidgetOverlayCollisionStaysStableAcrossZoomInteractionFrames() {
  const runtimePath = resolve(process.cwd(), 'src', 'components', 'StoryboardWidgetCanvas', 'runtime', 'useStoryboardWidgetOverlayCollision.ts')
  ...
  if (text.includes('STORYBOARD_WIDGET_INTERACTION_FRAME_EVENT')) {
...
export function testStoryboardWidgetOverlayCollisionSkipsSelfCommittedStoreChurn() {
  ...
  if (!text.includes('st.openWidgetNodeIdsByRenderer?.storyboard')) {
    throw new Error('expected overlay collision warmup and open-widget watcher to prefer Storyboard renderer-scoped widget ids')
  }
  if (!text.includes('Array.isArray(s.openWidgetNodeIdsByRenderer?.storyboard)')) {
    throw new Error('expected overlay collision open-widget subscription to read renderer-scoped Storyboard Widget ids before any global fallback')
  }
}
