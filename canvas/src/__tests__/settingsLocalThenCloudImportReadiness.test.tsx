...
function SettingsLocalThenCloudImportHarness(props: {
...
export async function testSettingsLocalThenCloudImportKeepsCommittedSurfaceTruthfulWhileLatestDraftWins() {
  ...
  const unregisterBridge = registerMarkdownWorkspaceActionBridge('test-local-then-cloud-import-bridge', {
    importUrl: url => {
  ...
  try {
    ...
    await mountReactRoot(settingsRoot, React.createElement(SettingsLocalThenCloudImportHarness, { actionsRef }), {
    ...
    if (useMarkdownExplorerStore.getState().activePath !== LOCAL_HISTORY_PATH) {
      throw new Error(`expected earlier local import to remain the active editor path, got ${String(useMarkdownExplorerStore.getState().activePath || '')}`)
    }
