...
export function testFlowEditorContainmentGroupLookupCachesPerScene() {
  const scene = {
    nodes: [],
    edges: [],
    nodeById: new Map(),
  ...
  const first = getCachedFlowEditorContainmentGroupLookup(scene)
  const second = getCachedFlowEditorContainmentGroupLookup(scene)
  if (!first || !second) throw new Error('expected Flow Editor containment group lookup to be available')
...
export function testFlowEditorRuntimeSceneReusesContainmentGroupLookup() {
  const runtimeSceneText = readFileSync(
    resolve(process.cwd(), 'src', 'components', 'FlowEditorCanvas', 'runtime', 'useFlowEditorRuntimeScene.ts'),
    'utf8',
  )
  if (!runtimeSceneText.includes('getCachedFlowEditorContainmentGroupLookup(scene)?.readContainmentGroupForNode(id)')) {
    throw new Error('expected Flow Editor runtime scene to reuse the cached containment group lookup')
  }
