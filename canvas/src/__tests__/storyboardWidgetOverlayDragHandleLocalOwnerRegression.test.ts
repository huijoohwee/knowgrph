...
export function testStoryboardWidgetOverlayDragHandleStaysLocalWithoutSpace() {
  ...
  if (!text.includes("resolved.kind === 'overlay' && !overlayPinnedToNode && overlayDragHandle && button === 0 && spacePanHeld !== true")) {
    throw new Error('expected unpinned Storyboard Widget overlay drag handles to remain local owner interactions unless Space-pan is explicit')
  }
}
