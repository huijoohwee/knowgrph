...
import {
  ...
  rippleMermaidGanttVideoSequenceTimelineFromBoundary,
...
export function testVideoSequenceElementActionsStaySourceBacked() {
  ...
  if (
    ...
    !bookmark?.code.includes('Source video bookmark : clip_source_bookmark_4, kgsrc_4_4, vert, kgpos_4, 0.001m') ||
  ...
  const rippleMoved = rippleMermaidGanttVideoSequenceTimelineFromBoundary({
    boundaryMinutes: 4,
    code: rippleCode,
    deltaMinutes: 1,
    rowLineIndex: 5,
  ...
  if (
    ...
    !rippleMoved?.includes('Intro : clip_intro, kgsrc_0_4, kgpos_0, 4m') ||
    !rippleMoved.includes('Scene : clip_scene, kgsrc_4_7, kgpos_6, 3m') ||
    !rippleMoved.includes('Outro : clip_outro, kgsrc_7_9, kgpos_9, 2m') ||
    snapped !== 3 ||
    dragSnapped !== 2.98 ||
    splitPoint !== 2.98
  ) {
    throw new Error(`expected ripple delete, ripple shifting, and drag-aware auto snapping: ${JSON.stringify({ dragSnapped, rippleDeleted, rippleMoved, snapped, splitPoint })}`)
  }
}
