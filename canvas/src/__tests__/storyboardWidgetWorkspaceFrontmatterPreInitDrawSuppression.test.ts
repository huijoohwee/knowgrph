...
export function testStoryboardWidgetWorkspaceFrontmatterPreInitSuppressionKeepsRenderableGraphsOffEarlyDrawPath() {
  const runtimePath = resolve(process.cwd(), 'src', 'components', 'FlowCanvas', 'useFlowCanvasRuntime.ts')
  const runtimeText = readFileSync(runtimePath, 'utf8')
  if (!runtimeText.includes("import { isStoryboardWidgetFrontmatterDocumentModeRequested } from '@/lib/graph/frontmatterMode'")) {
    throw new Error('expected Flow runtime workspace-open pre-init draw suppression to reuse the shared Storyboard Widget frontmatter document mode helper')
  }
