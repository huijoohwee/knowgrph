...
export function testFlowEditorFrontmatterFlowDefaultsUnpinnedAndCenteredLayout() {
  ...
  if (!text.includes("seededFrontmatterAutoWidgetsKeyRef")) {
    throw new Error('expected FlowEditorCanvas to seed default frontmatter-flow widget pin state')
  }
  if (!text.includes('setFlowWidgetPinnedByNodeId')) {
    throw new Error('expected FlowEditorCanvas to write unpinned widget defaults into store')
  }
