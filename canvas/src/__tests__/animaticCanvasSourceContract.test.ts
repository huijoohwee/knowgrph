...
export function testAnimaticCanvasRetainsReferencePlayerAndTimelineShellContract() {
  ...
  for (const snippet of [
    ...
    'markdownText: string',
    'dragState.markdownDocumentName !== markdownDocumentName || dragState.markdownText !== markdownText',
    ...
    'className="pointer-events-none absolute inset-y-0 left-0 flex items-center px-2 text-[10px] text-slate-600"',
    'className="pointer-events-none absolute inset-y-0 left-0"',
    'pointer-events-auto absolute rounded-[4px] border px-0 py-0',
    ...
    'handleLaneItemPointerStart(event, lane.id, beat, item.nodeId, item.title)',
