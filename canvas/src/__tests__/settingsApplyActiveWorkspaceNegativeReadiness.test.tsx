...
function SettingsActiveWorkspaceNegativeHarness(props: {
...
export async function testSettingsActiveWorkspaceApplyKeepsCommittedFloatingChatSurfaceWhenNoActiveMarkdownFileExists() {
  ...
  try {
    ...
    await mountReactRoot(settingsRoot, React.createElement(SettingsActiveWorkspaceNegativeHarness, { actionsRef }), {
    ...
    if (historyStatus !== NO_ACTIVE_MARKDOWN_STATUS || knowgrphStatus !== NO_ACTIVE_MARKDOWN_STATUS) {
      throw new Error(`expected no-active-file actions to surface the shared missing-active-markdown status, got ${JSON.stringify({ historyStatus, knowgrphStatus })}`)
    }
