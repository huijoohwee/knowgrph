...
import { fitStoryboardWidgetPinnedWidgets, readFrontmatterOverlayFitProxyScale } from '@/components/FlowCanvas/fitPinnedWidgets'
import {
...
export function testStoryboardWidgetFitIncludesPinnedWidgets() {
  const nodes = [
    {
      id: 'n1',
      type: 'Test',
  ...
  const withPinned = fitStoryboardWidgetPinnedWidgets({
...
export function testStoryboardWidgetFitUsesResolvedOverlayPanelSize() {
  const nodes = [
    {
      id: 'n1',
      type: 'Test',
  ...
  const common = {
    ...
    openWidgetNodeIds: ['n1'],
    pinnedById: { n1: true },
    worldPosById: { n1: { x: 0, y: 0 } },
    portExtraPadScreenPx: 0,
    graphData: null,
    fitOpts: fitOpts as never,
  }

  const widgetDefault = fitStoryboardWidgetPinnedWidgets(common)
  const resolvedPanel = fitStoryboardWidgetPinnedWidgets({
    ...common,
    readOverlayPanelSize: () => ({ width: WIDGET_BASE_SIZE.width * 2, height: WIDGET_BASE_SIZE.height * 2 }),
  })

  if (!(resolvedPanel.k < widgetDefault.k - 1e-6)) {
    throw new Error(`expected resolved overlay panel size to reduce initial fit scale, default=${widgetDefault.k} resolved=${resolvedPanel.k}`)
  }
}

export function testStoryboardWidgetFitCentersVisiblePinnedOverlayCollective() {
  const nodes = [
    {
      id: 'n1',
      type: 'Test',
...
export function testStoryboardWidgetFitDerivesFrontmatterOverlayIdsWhenOpenSetIsEmpty() {
  const fit = fitStoryboardWidgetPinnedWidgets({
    nodes: [
      {
        id: 'w-text',
...
export function testStoryboardWidgetFitUsesDenserFrontmatterOverlayProxy() {
  const baseArgs = {
    nodes: [
      {
        id: 'w-text',
  ...
  const generic = fitStoryboardWidgetPinnedWidgets({
    ...baseArgs,
    graphData: null,
  })
  const frontmatter = fitStoryboardWidgetPinnedWidgets({
    ...baseArgs,
    graphData: {
      type: 'application/json',
...
export function testFrontmatterOverlayFitAvoidsTinyGraphFitBootstrapWhenPinnedNodesSpanHugeScene() {
  ...
  const frontmatter = fitStoryboardWidgetPinnedWidgets(baseArgs)
