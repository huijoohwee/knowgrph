...
export function testFlowEditorPinnedQuickEditorDragDoesNotProxyNodeDrag() {
  ...
  if (text.includes("const overlayNodeId = String((resolved.overlayRoot as HTMLElement).dataset?.kgNodeQuickEditor")) {
    throw new Error('expected FlowCanvas to avoid proxying pinned quick editor drag into node drag')
  }
}
