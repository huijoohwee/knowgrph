/**
 * Preservation Property Tests — Flow Editor Run Button Regression
 *
 ...
 */
export function testFlowEditorRunButtonPreservation_OpenFieldEditor_DescriptorContract() {
  ...
  const overlaySurfacePath = resolve(root, 'src', 'components', 'FlowEditorCanvas', 'runtime', 'flowEditorOverlaySurfaceElements.tsx')
  ...
  if (!overlaySurfaceText.includes('void args.runWorkflowNode(actionNodeId)')) {
    throw new Error(
      'Preservation failure: flowEditorOverlaySurfaceElements.tsx no longer calls runWorkflowNode(actionNodeId). ' +
 ...
 */
export function testFlowEditorRunButtonPreservation_ClearOutputs_DescriptorContract() {
 ...
 */
export function testFlowEditorRunButtonPreservation_ComputeCompletion_SideEffectsContract() {
 ...
 */
export function testFlowEditorRunButtonPreservation_ButtonRendering_WidgetCardActionsContract() {
 ...
 * **Validates: Requirements 3.5**
 */
export function testFlowEditorRunButtonPreservation_DownloadTypeTrigger_IsNotBugCondition() {
 ...
 * **Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5**
 */
export function testFlowEditorRunButtonPreservation_PBT_NonDownloadTriggers_IsBugConditionFalse() {
 ...
 * - isFlowEditorWorkflowRunnableNode returns a boolean (never crashes on unknown node types)
 * - resolveFlowEditorWorkflowDownstreamRunTargetIds returns an array (never undefined)
 ...
 * **Validates: Requirements 3.3, 3.4**
 */
export function testFlowEditorRunButtonPreservation_PBT_DispatchTableCompleteness() {
  const root = process.cwd()

  // Read the downstream run targets module
  const runnableTargetsPath = resolve(root, 'src', 'components', 'FlowEditorCanvas', 'runtime', 'flowEditorWorkflowDownstreamRunTargets.ts')
  const runnableTargetsText = readFileSync(runnableTargetsPath, 'utf8')

  // Verify isFlowEditorWorkflowRunnableNode is exported (dispatch infrastructure completeness)
  if (!runnableTargetsText.includes('export function isFlowEditorWorkflowRunnableNode')) {
    throw new Error(
      'PBT dispatch table completeness failure: isFlowEditorWorkflowRunnableNode is not exported from flowEditorWorkflowDownstreamRunTargets.ts. ' +
      'Requirement 3.3: the runnability check must be available to the dispatcher.'
    )
  }

  // Verify resolveFlowEditorWorkflowDownstreamRunTargetIds is exported
  if (!runnableTargetsText.includes('export function resolveFlowEditorWorkflowDownstreamRunTargetIds')) {
    throw new Error(
      'PBT dispatch table completeness failure: resolveFlowEditorWorkflowDownstreamRunTargetIds is not exported. ' +
  ...
  if (!runnabilityCheckReturnsBoolean) {
    throw new Error(
      'PBT dispatch table completeness failure: isFlowEditorWorkflowRunnableNode does not declare a boolean return type. ' +
  ...
  if (!targetResolverReturnsArray) {
    throw new Error(
      'PBT dispatch table completeness failure: resolveFlowEditorWorkflowDownstreamRunTargetIds does not declare a string[] return type. ' +
  ...
  if (!overlaySurfaceText.includes('runWorkflowNode')) {
    throw new Error(
      'PBT dispatch table completeness failure: flowEditorOverlaySurfaceElements.tsx does not reference runWorkflowNode. ' +
 ...
 * **Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5**
 */
export function testFlowEditorRunButtonPreservation_PBT_BehavioralIdentity() {
 ...
 * - Both feed into resolveFlowEditorWorkflowDownstreamRunTargetIds
 *
 * **Validates: Requirements 3.3**
 */
export function testFlowEditorRunButtonPreservation_RunDownstream_TargetResolutionContract() {
  const root = process.cwd()
  ...
  if (!runnableTargetsText.includes("'canvas:widgetCard'")) {
    throw new Error(
      'Preservation failure: flowEditorWorkflowDownstreamRunTargets.ts does not reference "canvas:widgetCard". ' +
  ...
  if (!runnableTargetsText.includes("!== 'runDownstream'")) {
    throw new Error(
      "Preservation failure: flowEditorWorkflowDownstreamRunTargets.ts does not check for 'runDownstream' trigger. " +
 ...
 */
export function testFlowEditorRunButtonPreservation_ExportWorkflowBundle_FunctionPreserved() {
  const root = process.cwd()
  const workflowActionsPath = resolve(root, 'src', 'components', 'FlowEditorCanvas', 'runtime', 'useFlowEditorWorkflowActions.ts')
  ...
  if (!workflowActionsText.includes('exportWorkflowBundle')) {
    throw new Error(
      'Preservation failure: exportWorkflowBundle function no longer exists in useFlowEditorWorkflowActions.ts. ' +
  ...
  if (!workflowActionsText.includes('return { exportWorkflowBundle, runWorkflowNode }')) {
    throw new Error(
      'Preservation failure: useFlowEditorWorkflowActions no longer returns { exportWorkflowBundle, runWorkflowNode }. ' +
