...
export function testTreePresetPreservesSchemaOverridesForOrientationAndSeparation() {
  ...
  const next = computeNextSchemaForTreePreset(base, 'mermaid', [
    'hasSection',
    'hasBlock',
    'hasItem',
    'hasMermaid',
  ...
  const tree = next.layout?.tree
  if (!tree) {
    throw new Error('expected layout.tree to be present after preset toggle')
  }
  if (tree.orientation !== 'vertical') {
    throw new Error(`expected schema orientation "vertical" to be preserved, got ${String(tree.orientation)}`)
  }
  if (tree.separation !== 2.5) {
    throw new Error(`expected schema separation 2.5 to be preserved, got ${String(tree.separation)}`)
  }
  if (tree.direction !== 'source-target') {
    throw new Error(`expected schema direction "source-target" to be preserved, got ${String(tree.direction)}`)
  }
}

export function testTreePresetUsesMetadataWhenSchemaOrientationAndSeparationUnset() {
  const base: GraphSchema = {
    ...defaultSchema,
    layout: {
      ...(defaultSchema.layout || {}),
  ...
  if (tree.orientation !== 'vertical') {
    throw new Error(`expected metadata orientation "vertical" to be applied, got ${String(tree.orientation)}`)
  }
  if (tree.separation !== 1.7) {
    throw new Error(`expected metadata separation 1.7 to be applied, got ${String(tree.separation)}`)
  }
}
