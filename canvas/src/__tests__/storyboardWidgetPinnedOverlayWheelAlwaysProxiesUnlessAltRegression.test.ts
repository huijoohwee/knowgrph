...
export function testStoryboardWidgetPinnedOverlayWheelAlwaysProxiesUnlessAlt() {
  ...
  if (!text.includes('opts?.storyboardWidgetMode')) {
    throw new Error('expected overlay wheel proxy to accept StoryboardWidget context')
  }
  const iPinned = text.indexOf('storyboardWidgetMode && overlayPinnedToNode')
  ...
  if (!(iExplicitZoomIntent < iScroll && iScroll < iPinned)) {
    throw new Error('expected explicit ctrl/cmd canvas zoom to bypass widget scroll while ordinary wheel checks scroll before pinned proxy')
  }
