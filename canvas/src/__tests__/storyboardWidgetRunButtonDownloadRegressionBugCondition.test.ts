/**
 * Bug Condition Exploration Test — Storyboard Widget Run Button Download Regression
...
const runnableTargetsText = readFileSync(resolve(root, 'src/components/StoryboardWidgetCanvas/runtime/storyboardWidgetWorkflowDownstreamRunTargets.ts'), 'utf8')
...
export function testStoryboardWidgetRunButtonBugCondition_CandidateA_DomAncestorProbe() {
  if (
...
export function testStoryboardWidgetRunButtonBugCondition_CandidateB_DispatcherBlobBranchProbe() {
  if (
  ...
  const runnabilityMissingInputWidget =
    runnableTargetsText.includes('isStoryboardWidgetWorkflowRunnableNode') &&
...
export function testStoryboardWidgetRunButtonBugCondition_CoreAssertion_isBugConditionHolds() {
