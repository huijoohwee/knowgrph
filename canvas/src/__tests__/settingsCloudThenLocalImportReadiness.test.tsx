...
const CLOUD_KNOWGRPH_URL = 'https://cloud.example/knowgrph-overlap-cloud.md'
const CLOUD_HISTORY_URL = 'https://cloud.example/history-overlap-cloud.md'
const LOCAL_KNOWGRPH_FILE_NAME = 'kgc_20260523171500.md'
...
export async function testSettingsCloudThenLocalImportKeepsCommittedSurfaceTruthfulWhileLatestDraftWins() {
  ...
  try {
    ...
    store.setChatKnowgrphCloudUrl('https://cloud.example/knowgrph-before-cloud-local.md')
    ...
    if (knowgrphStatus !== LOCAL_KNOWGRPH_PATH || historyStatus !== LOCAL_HISTORY_PATH) {
      throw new Error(`expected later local-import statuses to win over earlier cloud-import statuses, got ${JSON.stringify({ knowgrphStatus, historyStatus })}`)
    }
    if (
    ...
    if (
      ...
      preApplyInspection.cloudUrls.chatKnowgrphCloudUrl !== 'https://cloud.example/knowgrph-before-cloud-local.md' ||
