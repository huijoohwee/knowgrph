...
export function testFrontmatterOnlyPolicySkipsKeywordTableAndComposedSourceDerivations() {
  ...
  if (!activeGraphDataText.includes('const effectiveMultiDimTableModeEnabled = frontmatterOnlyPolicyActive ? false : multiDimTableModeEnabled')) {
    throw new Error('expected frontmatter-only policy to disable table derivation mode')
  }
  if (!activeGraphDataText.includes('multiDimTableModeEnabled: effectiveMultiDimTableModeEnabled')) {
