...
export function testFlowEditorNodeDraftUpdatesRefBeforeRunStoreWriteback() {
  ...
  const draftWrite = 'args.draftGraphDataRef.current = bumpFlowEditorDraftGraphDataRevision(normalizeGraphData({ ...draft, nodes }))'
...
export function testFlowEditorAutoRunSchedulesAfterWidgetPropertyCommit() {
  ...
  const callback = 'const handleNodePropertiesCommittedForAutoRun = React.useCallback((nodeId: string) => {'
  ...
  const callbackProp = 'onNodePropertiesCommittedForAutoRun?: (nodeId: string) => void'
  ...
  for (const [source, snippet] of [
    ...
    [actionsText, 'args.onNodePropertiesCommittedForAutoRun?.(id)'],
  ...
  if (patchWrapperBlock.indexOf('updateNodeById(id, { properties: nextProps as never })') > patchWrapperBlock.indexOf('args.onNodePropertiesCommittedForAutoRun?.(id)')) {
    throw new Error('expected property patch auto-run callback after live draft ref update')
  }
  if (setWrapperBlock.indexOf('updateNodeById(id, { properties: (properties || {}) as never })') > setWrapperBlock.indexOf('args.onNodePropertiesCommittedForAutoRun?.(id)')) {
...
export function testFlowEditorRunCommitsActiveSharedInlineEditorBeforeRun() {
  ...
  if (!overlayElementsText.includes('const graphDataForRunResolution = useStableFrontmatterGraphAuthority')
