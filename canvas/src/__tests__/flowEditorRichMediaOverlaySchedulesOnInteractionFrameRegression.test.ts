...
export function testFlowCanvasSchedulesRichMediaOverlayOnInteractionFrame() {
  ...
  if (!flowText.includes('if (isFlowEditor) mediaOverlayInteractionFrameSchedulerRef.current?.()')) {
    throw new Error('expected FlowCanvas to schedule rich-media layout from live Flow Editor shared surface interaction frames without affecting Flow Canvas')
  }
  ...
  if (!overlaysText.includes('if (!flowEditorSharedSurfaceRendererMode)')) {
    throw new Error('expected rich-media interaction-frame scheduling to stay scoped to Flow Editor shared surfaces')
  }
