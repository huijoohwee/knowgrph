...
export function testStoryboardWidgetInitialTransformDoesNotReapplyAfterUserPan() {
  ...
  if (shouldApply({ datasetKey: 'd1', zoomViewKey: 'k1', current: d3.zoomIdentity.translate(10, 0).scale(1), canvas2dRenderer: 'storyboard' })) {
    throw new Error('expected subsequent apply to be blocked after user transform')
  }
  if (shouldApply({ datasetKey: 'd1', zoomViewKey: 'k2', current: d3.zoomIdentity.translate(10, 0).scale(1), canvas2dRenderer: 'storyboard' })) {
    throw new Error('expected storyboardWidget init key to ignore zoomViewKey churn')
  }
  if (!shouldApply({ datasetKey: 'd2', zoomViewKey: 'k2', current: d3.zoomIdentity.translate(10, 0).scale(1), canvas2dRenderer: 'storyboard' })) {
    throw new Error('expected apply to be allowed for new datasetKey')
  }

  if (!shouldApply({ datasetKey: 'rev:1', zoomViewKey: 'k3', current: d3.zoomIdentity, canvas2dRenderer: 'storyboard' })) {
    throw new Error('expected apply to be allowed for rev datasetKey')
  }
  if (shouldApply({ datasetKey: 'rev:2', zoomViewKey: 'k4', current: d3.zoomIdentity, canvas2dRenderer: 'storyboard' })) {
    throw new Error('expected rev datasetKey changes to be ignored for storyboardWidget init key')
  }
