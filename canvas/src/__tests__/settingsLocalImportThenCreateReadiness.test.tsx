...
const IMPORTED_KNOWGRPH_FILE_NAME = 'kgc_20260523170900.md'
...
const CREATED_KNOWGRPH_PATH = '/workspace/chat/kgc_20260523171000.md'
...
export async function testSettingsLocalImportThenCreateKeepsCommittedSurfaceTruthfulWhileLatestDraftWins() {
  ...
  try {
    ...
    store.setChatKnowgrphCloudUrl('https://cloud.example/knowgrph-before-import-create.md')
    ...
    if (knowgrphStatus !== CREATED_KNOWGRPH_PATH || historyStatus !== CREATED_HISTORY_PATH) {
      throw new Error(`expected later create-file statuses to win over earlier local-import statuses, got ${JSON.stringify({ knowgrphStatus, historyStatus })}`)
    }
    if (
    ...
    if (
      ...
      preApplyInspection.cloudUrls.chatKnowgrphCloudUrl !== 'https://cloud.example/knowgrph-before-import-create.md' ||
