...
import { syncGraphDataToGraphRecordDb } from '@/lib/graph-record-db'
import type { GraphData } from '@/lib/graph/types'
import { cancelWorkspaceSyncTask, scheduleWorkspaceSyncTask } from '@/lib/async/workspaceSyncScheduler'
import { WORKSPACE_SYNC_SCOPE_GRAPH_RECORD_RUNTIME_PERSISTENCE } from '@/lib/async/workspaceSyncKeys'

type UseGraphRecordDbSyncResult = {
...
const toSyncTaskKey = (viewKey: string): string => {
  ...
  return `graph-record:runtime-persistence-sync:${key}`
}

const toSyncScopeKey = (viewKey: string): string => {
  const key = String(viewKey || '').trim() || 'default'
  return `${WORKSPACE_SYNC_SCOPE_GRAPH_RECORD_RUNTIME_PERSISTENCE}:${key}`
}

const scheduleGraphRecordSync = (viewKey: string, gate: SyncGate) => {
  ...
  scheduleWorkspaceSyncTask(taskKey, () => {
    ...
    gate.inFlight = syncGraphDataToGraphRecordDb(gate.latestGraphData || null)
      .then(() => {
      ...
      .finally(() => {
        ...
        scheduleGraphRecordSync(viewKey, gate)
      })
  }, 140, {
