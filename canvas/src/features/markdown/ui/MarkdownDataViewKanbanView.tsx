...
export const MarkdownDataViewKanbanView = React.memo(function MarkdownDataViewKanbanView(props: MarkdownDataViewKanbanViewProps) {
  ...
  const groups = React.useMemo(() => {
    ...
    for (const r of view.rows) {
      const key = String(r.cells[groupByIndex] ?? '').trim() || 'Ungrouped'
  ...
  return (
    <section className="p-2 overflow-x-auto" aria-label="Kanban view">
          ...
          >
            ...
            <div className="p-2 space-y-2">
              {g.rows.map(r => {
                ...
                const moveTargets = groups.map(x => x.key).filter(Boolean)
                return (
                  <article
                    key={r.id}
                    className={[
                      ...
                      UI_THEME_TOKENS.panel.border,
                      'shadow-sm',
                      onActivateRow ? 'cursor-pointer hover:bg-[var(--kg-panel-bg-hover)] focus:outline-none focus:ring-2 focus:ring-blue-500/50' : '',
                    ].join(' ')}
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className={['text-xs font-semibold leading-5', UI_THEME_TOKENS.text.primary].join(' ')}>
                    ...
                    {canMutate ? (
                      <div className="mt-2">
                        ...
                        >
                          {(view.columns[groupByIndex].options || [g.key]).map(o => (
