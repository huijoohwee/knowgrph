...
export function buildProbeTreeCardMaterializationPrompt(userQuery: string): string {
  ...
  return [
    ...
    '- The local no-model path never synthesizes clarification cards. It may project exactly one Type 2 card only when the selected child literally authors a 2-4 alternative list; preserve its question and option labels verbatim and mark probeTreeDerivation: authored-choices.',
    '- If neither that literal authored-choice structure nor an approved/configured model yielding 2-4 distinct cards exists, fail closed as insufficient context instead of producing canned, hardcoded, or entity-to-template content.',
