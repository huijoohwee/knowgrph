...
export const buildChatResponseStructuredSurfaceBlock = (responseSurface?: ChatResponseStructuredSurface | null): string => {
  ...
  return rows.length > 0 ? ['### MCP Structured Response Projection', '', 'Assistant structured-content entries are projected into ordinary Storyboard Widget nodes and Rich Media Panel nodes, so text, image, audio, video, HTML, cards, and edges reuse the same Storyboard Widget, Card, panel, and inline-edit owners.', '', '| Node | Kind | Render fields | Incoming edge |', '|---|---|---|---|', ...rows, ''].join('\n') : ''
}
