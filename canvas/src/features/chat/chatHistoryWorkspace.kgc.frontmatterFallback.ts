...
export const buildFrontmatter = (args: {
  ...
  return [
    ...
    `    sys_event: ${JSON.stringify(`validateArtifact() runs V-01–V-07 plus V-10 generated-table persistence and coverage checks for ${validationFocus.join(', ') || 'request-specific requirements'}; first failure emits correction back to n-process via feedback arc e5; pass emits valid_md to n-deliver`)}`,
    '    data_in: "md"',
    ...
    `      data:          {key: data,          type: object,   value: {rules: ["V-01","V-02","V-03","V-04","V-05","V-06","V-07","V-10"], max_retry: 3, required_sections: ${JSON.stringify(requiredSections)}, coverage_terms: ${JSON.stringify(validationFocus)}}}`,
    '      applies_rules: {key: applies_rules, type: array,    value: ["V-01","V-02","V-03","V-04","V-05","V-06","V-07","V-10"]}',
    '      db_writes:     {key: db_writes,     type: string,   value: "flow_nodes"}',
