...
interface RenderBottomPanelSections {
  byKey: {
    links: boolean
    layout: boolean
...
export interface RenderBottomPanelState {
  renderUiEditorOpen: boolean
  setRenderUiEditorOpen: (next: boolean) => void
  sections: RenderBottomPanelSections
  allSectionsCollapsed: boolean
  collapseAllSections: () => void
  expandAllSections: () => void
}

type RenderSectionToggleSource = 'bottomPanel' | 'floatingPanel'
...
export function useRenderBottomPanelState(options?: { source?: RenderSectionToggleSource }): RenderBottomPanelState {
  const source: RenderSectionToggleSource = options?.source ?? 'bottomPanel'
  ...
  const sections: RenderBottomPanelSections = useMemo(
    () => ({
      byKey: {
