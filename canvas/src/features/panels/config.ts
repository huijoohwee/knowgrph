...
export const WORKFLOW_STEP_COPY: Record<WorkflowStepId, WorkflowStepCopy> = {
  ...
  8: {
    ...
    descriptionLong:
      'Render graphs in 2D/3D or map views, or feed exports into indexers, dashboards, and downstream RAG pipelines so appearance and retrieval strategies remain late, replaceable decisions on top of stable GraphData. Visual mappings, level-of-detail, canvas Embed/Overlay mini-visualizations, and schema-driven group polygons (controlled by the Group Polygons toolbar toggle and `schema.metadata["canvas:polygons"]`) happen here without altering the canonical store, keeping phase/step-style outlines and overlays domain-agnostic.',
  },
}
...
export const HELP_SHORTCUT_ITEMS: readonly string[] = [
  ...
  'Markdown: MDX slides allow JSX components in the markdown body (e.g., Alert, Chart, LiveCode, Mermaid).',
