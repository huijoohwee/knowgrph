...
export function useMarkdownLoader(
  ...
  React.useEffect(() => {
    ...
    if (!url) {
      ...
      setIsLoading(false)
      setLoadError(UI_COPY.markdownWorkspaceMissingPathError)
      return
    }
    setIsLoading(true)
    setLoadError(null)
    const load = async () => {
      try {
        ...
        if (looksLikeViteDevIndexHtml(text)) {
          throw new Error(UI_COPY.markdownWorkspaceLoadFailedError)
        }
        if (cancelled) return
        const baseName = (() => {
          const raw = basePath.split(/[/\\]/).pop() || ''
        ...
        setLoadError(message || UI_COPY.markdownWorkspaceLoadFailedError)
      } finally {
        if (!cancelled) {
          setIsLoading(false)
        }
