...
import { isStoryboardCanvas2dRenderer, type Canvas2dRendererId } from '@/lib/config.render'
...
export const buildMinimapStoryboardWidgetOverlayNodeById = (args: {
  canvas2dRenderer: Canvas2dRendererId | null | undefined
  nodes: GraphNode[]
}): Map<string, GraphNode> | null => {
  const { canvas2dRenderer, nodes } = args
  if (!canvas2dRenderer || !isStoryboardCanvas2dRenderer(canvas2dRenderer) || nodes.length === 0) return null
  const graphData = { type: 'application/json', nodes, edges: [] as GraphEdge[] }
  const graphSemanticKey = buildScopedGraphSemanticKey('minimap-storyboard-widget-overlay-subset', {
    graphData,
  ...
  const lookup = getCachedGraphLookup({
    cacheScope: 'minimap-storyboard-widget-overlay-subset',
    graphData,
    graphSemanticKey,
  })
...
export const buildMinimapStoryboardWidgetOverlaySubset = (args: {
  canvas2dRenderer: Canvas2dRendererId | null | undefined
  canvasDims: CanvasDimensionsLike
  edges: GraphEdge[]
  storyboardWidgetOverlayNodeById: Map<string, GraphNode> | null
  ...
  if (!args.canvas2dRenderer || !isStoryboardCanvas2dRenderer(args.canvas2dRenderer) || ids.length === 0) return null
  ...
  const nodeById = args.storyboardWidgetOverlayNodeById || new Map<string, GraphNode>()
