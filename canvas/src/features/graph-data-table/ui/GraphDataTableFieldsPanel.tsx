...
export function FieldsPanel({
  ...
  return (
    <div className={`z-50 flex max-h-96 w-80 max-w-[calc(100vw-24px)] flex-col overflow-hidden rounded-lg border ${UI_THEME_TOKENS.panel.border} ${UI_THEME_TOKENS.panel.bg} p-4 ${UI_THEME_TOKENS.text.primary} shadow-md sm:w-96 md:w-[544px] ${uiPanelTextFontClass}`}>
      ...
      <div className={`flex-1 overflow-auto border ${UI_THEME_TOKENS.panel.border} rounded-md ${UI_THEME_TOKENS.panel.headerBg}`}>
        {fieldsPanelColumnKeys.length === 0 ? (
          <div className={`flex h-32 items-center justify-center ${uiPanelKeyValueTextSizeClass} ${UI_THEME_TOKENS.text.tertiary}`}>{UI_COPY.noFieldsMatch}</div>
        ) : (
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {fieldsPanelColumnKeys.map(key => {
              ...
              return (
                <label
                  key={key}
                  className={`flex items-center justify-between gap-2 px-2 py-1.5 ${uiPanelKeyValueTextSizeClass} ${
