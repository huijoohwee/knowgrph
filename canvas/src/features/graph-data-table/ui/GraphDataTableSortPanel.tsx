...
export function SortPanel({
  ...
  return (
    <div className={`z-50 border ${UI_THEME_TOKENS.panel.border} ${UI_THEME_TOKENS.panel.bg} ${UI_THEME_TOKENS.text.primary} shadow-md outline-none flex max-h-96 w-80 sm:w-96 md:w-[544px] max-w-[calc(100vw-24px)] flex-col overflow-hidden rounded-lg p-4 relative ${uiPanelTextFontClass}`}>
      ...
      <div className={`mb-3 flex items-center justify-between gap-2 ${uiPanelKeyValueTextSizeClass}`}>
      ...
      <div className="flex flex-1 flex-col gap-2 overflow-auto pt-2 pb-4">
        {sortRules.length === 0 ? (
          <div className={`${uiPanelKeyValueTextSizeClass} ${UI_THEME_TOKENS.text.tertiary}`}>{UI_COPY.graphDataTableSortingByIdFallbackLabel}</div>
        ) : (
          sortRules.map(rule => (
            <div key={rule.id} className="flex items-center gap-2">
              <FilterCombobox
                ...
                className={`justify-center whitespace-nowrap rounded-md font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500 disabled:pointer-events-none disabled:opacity-50 gap-2 border ${UI_THEME_TOKENS.panel.border} ${UI_THEME_TOKENS.panel.bg} shadow-sm ${UI_THEME_TOKENS.button.hoverBg} px-2 py-1 ${uiPanelKeyValueTextSizeClass}`}
              />
              <FilterCombobox
                value={rule.dir}
