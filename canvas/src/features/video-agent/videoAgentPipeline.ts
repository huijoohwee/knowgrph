...
import { buildVideoAgentDatasetRuntime, type VideoAgentDatasetRuntime } from './videoAgentDatasetRuntime'
import {
  CAPABILITY_META,
  DEFAULT_CAPABILITIES,
  REASONING_META,
} from './videoAgentPipelineMeta'
...
export function buildVideoAgentPipeline(input: VideoAgentPipelineInput): VideoAgentPipelineResult {
  ...
  const data = {
    ...
    workspaceFiles: [
      ...
      { path: 'video-agent/visual-dataset.json', kind: 'json', role: 'visual-annotation-dataset' },
