...
import { applyMappingRowsToRegistryEntry, buildMappingRowsFromRegistryEntry, validateMappingRows, type StoryboardWidgetMappingRow } from '@/features/storyboard-widget-manager/mappingRows'
import { StoryboardWidgetMappingTabLayout } from '@/features/storyboard-widget-manager/StoryboardWidgetMappingTabLayout'
import { exportWidgetRegistryAsJson, importWidgetRegistryFromJson } from '@/features/storyboard-widget-manager/StoryboardWidgetMappingRegistryIo'
import { getDefaultSmartMediaModel, inferSmartMediaMode } from '@/features/storyboard-widget-manager/StoryboardWidgetMappingSmartMedia'
import { useStoryboardWidgetMappingRowsEditor } from '@/features/storyboard-widget-manager/useStoryboardWidgetMappingRowsEditor'

export default function StoryboardWidgetMappingTab({ searchQuery, onRegisterActions }: {
  searchQuery: string
  ...
  const [editorRows, setEditorRows] = React.useState<StoryboardWidgetMappingRow[]>(() =>
  ...
  const openCreateFromWidget = React.useCallback(() => {
    ...
    const nodeId = String(selectedNodeId || '').trim()
    if (!cur || !nodeId) {
      upsertUiToast({ id: 'storyboard-widget-manager-no-selected-node', kind: 'warning', message: 'Select a node first.', ttlMs: 2500 })
      return
    }
    const node = (cur.nodes || []).find(n => n && n.id === nodeId) || null
    const nodeTypeId = String(node?.type || '').trim()
    if (!nodeTypeId) {
      upsertUiToast({ id: 'storyboard-widget-manager-selected-node-missing-type', kind: 'warning', message: 'Selected node has no type.', ttlMs: 2500 })
      return
    }
  ...
  const registerGenerateVideoFromSelection = React.useCallback(() => {
    ...
    const nodeId = String(selectedNodeId || '').trim()
    if (!cur || !nodeId) {
      upsertUiToast({ id: 'storyboard-widget-manager-no-selected-node-gv', kind: 'warning', message: 'Select a node first.', ttlMs: 2500 })
      return
    }
    const node = (cur.nodes || []).find(n => n && n.id === nodeId) || null
    if (!node) return
    ...
    if (res.ok !== true) {
      upsertUiToast({
        id: 'storyboard-widget-manager-register-gv-failed',
        kind: 'warning',
    ...
    upsertUiToast({ id: 'storyboard-widget-manager-register-gv-ok', kind: 'neutral', message: 'Registered Generate Video mapping.', ttlMs: 2500 })
  ...
  const registerSelectedNodeTypeFromSelection = React.useCallback(() => {
    ...
    const nodeId = String(selectedNodeId || '').trim()
    if (!cur || !nodeId) {
      upsertUiToast({ id: 'storyboard-widget-manager-no-selected-node-register', kind: 'warning', message: 'Select a node first.', ttlMs: 2500 })
      return
    }
    const node = (cur.nodes || []).find(n => n && n.id === nodeId) || null
    if (!node) return
    ...
    if (!baseType) {
      upsertUiToast({ id: 'storyboard-widget-manager-selected-node-missing-type-register', kind: 'warning', message: 'Selected node has no type.', ttlMs: 2500 })
      return
    }
    ...
    if (res.ok !== true) {
      upsertUiToast({
        id: 'storyboard-widget-manager-register-selected-type-failed',
        kind: 'warning',
        message: 'message' in res ? String(res.message || '').trim() : 'Register failed.',
        ttlMs: 4000,
      })
    ...
    setSelectedId(res.id)
    setSelectionMode('manual')
    upsertUiToast({ id: 'storyboard-widget-manager-register-selected-type-ok', kind: 'neutral', message: 'Registered mapping.', ttlMs: 2200 })
  }, [graphData, selectedNodeId, updateNode, upsertWidgetRegistryEntry, upsertUiToast])
  ...
  const applySelectedMappingToSelectedNode = React.useCallback(() => {
    ...
    const nodeId = String(selectedNodeId || '').trim()
    if (!entry) {
      upsertUiToast({ id: 'storyboard-widget-manager-apply-no-selected-entry', kind: 'warning', message: 'Select a mapping first.', ttlMs: 2500 })
      return
    }
    if (!cur || !nodeId) {
      upsertUiToast({ id: 'storyboard-widget-manager-apply-no-selected-node', kind: 'warning', message: 'Select a node first.', ttlMs: 2500 })
      return
    }
    const node = (cur.nodes || []).find(n => n && n.id === nodeId) || null
    if (!node) return
    ...
    upsertUiToast({ id: 'storyboard-widget-manager-apply-ok', kind: 'neutral', message: 'Applied mapping to node.', ttlMs: 2200 })
  ...
  const {
    ...
    deleteEditorRow,
    reorderEditorRow,
  } = useStoryboardWidgetMappingRowsEditor(setEditorRows)
