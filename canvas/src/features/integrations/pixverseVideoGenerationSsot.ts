...
export const PIXVERSE_VIDEO_GENERATION_DOC_ROWS: ReadonlyArray<PixVerseVideoDocRow> = [
  {
    ...
    typeLabel: 'string',
    value: 'pixverse',
    responsibility: 'Harness provider mode used by the local super-agent rich-media pipeline.',
    notes: 'Run the local harness with `--provider-mode pixverse` to activate the PixVerse MCP stdio adapter.',
    searchHints: ['provider_mode pixverse superagent harness'],
  },
  {
    key: 'strategy',
    typeLabel: 'enum',
    value: 'auto | text-to-video | image-to-video | transition-video | fusion-video',
    responsibility: 'Generation strategy selector for the PixVerse harness adapter.',
  ...
  },
  {
    ...
    typeLabel: 'enum',
    value: '5 | 8',
    responsibility: 'Allowed PixVerse generation durations for the shipped harness path.',
  ...
  },
  {
    ...
    notes: 'Knowgrph now supports generated-video and uploaded-video lip sync through the harness boundary, with TTS, explicit media ids, and automated local video/audio upload when the PixVerse MCP build exposes the required upload tools.',
  ...
  },
  {
    ...
    notes: 'Knowgrph enables this only through the harness boundary, with prompt, original-audio retention, explicit uploaded video ids, and automated local video upload controlled by environment variables. The current safe slice keeps sound effects and lip sync mutually exclusive per run.',
...
export const PIXVERSE_VIDEO_GENERATION_API_DOC_ENTRIES: ReadonlyArray<VirtualSettingsEntry> =
  PIXVERSE_VIDEO_GENERATION_DOC_ROWS.map(row => ({
    ...
    searchHints: ['pixverse video generation local harness mcp stdio', row.key, ...(row.searchHints || [])],
    details: {
      area: PIXVERSE_VIDEO_GENERATION_API_DOC_AREA,
      responsibility: row.responsibility,
      notes: String(row.notes || '').trim(),
      modules: ['knowgrph_parser/superagent_pixverse.py'],
      classes: ['PixVerseMcpStdioClient'],
      functions: ['run_pixverse_text_to_video'],
