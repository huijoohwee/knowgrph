...
const OPENAI_DOC_ROW_BY_ROW_KEY: Readonly<Record<string, OpenAiApiDocRow>> = {
  'openaiApi.provider': {
    ...
    responsibility: 'Orchestrator -> pin OpenAI provider routing -> keep Integrations, Workflow Manager, and the Text Widget configured for OpenAI on the same provider family.',
    notes: 'Integration transport setting reused by the Text Widget when configured for OpenAI.',
  ...
  'openaiApi.auth_mode': {
    ...
    notes: 'Server-managed auth reads KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY or OPENAI_API_KEY from the proxy runtime; BYOK stays memory-only.',
    searchHints: ['chatAuthMode auth byok serverManaged api key Cloudflare secret KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY OPENAI_API_KEY'],
  ...
  'openaiApi.api_key': {
    ...
    valueKey: 'chatApiKey',
    responsibility: 'Credential manager -> use proxy-owned OpenAI secrets by default, or hold caller-supplied OpenAI secrets in memory only for explicit BYOK runs.',
    notes: 'Server-managed mode leaves this field empty and lets the proxy use KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY or OPENAI_API_KEY.',
    searchHints: ['chatApiKey api key authentication bearer memory-only server-managed proxy KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY OPENAI_API_KEY'],
    modules: ['canvas/src/hooks/store/uiSlice.ts', 'canvas/src/features/settings/registry-ui.ui.ts', 'canvas/src/components/StoryboardWidgetCanvas.tsx'],
    classes: ['GraphState', 'RunGenerationConfig'],
