  ...
  const contracts = [
    ...
    ...(includeBrowserOnlyTools
      ? [{
          ...
          description: 'Apply, clear, configure cast/camera marks, play, pause, scrub, or export native XR choreography through structured fields or the upstream /animation.control, #character-motion, #action-path, @selected-actor, and @canvas grammar.',
          inputSchema: {
            type: 'object',
            additionalProperties: false,
            properties: {
              ...
              operation: { type: 'string', enum: ['apply', 'clear', 'configure-mark', 'play', 'pause', 'scrub', 'export'] },
