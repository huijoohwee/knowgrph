  ...
  const contracts = [
    ...
    ...(includeBrowserOnlyTools
      ? [{
          ...
          description: 'Control shared Camera framing, keyboard orbit, subject-bound XR moves, and camera choreography through structured actions or upstream Camera commands, bindings, semantic routes, and typed key=value parameters.',
          inputSchema: {
            type: 'object',
            additionalProperties: false,
            properties: {
              invocation: { type: 'string', description: 'Invocation such as /camera.animate @camera #camera-motion keys=w+d amount=0.08.' },
