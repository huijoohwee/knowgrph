...
7. `knowgrph.html_video.render`
   ...
   - Probe-Tree chat/model responses use `probe-tree-llm-response/v3` under `response.structuredContent`: one source Widget anchor, 2-4 Type 2 cards with `parentNodeId`, 2-4 numbered `selectionOptions`, 2-6 verbatim `contextAnchors` copied from the selected user input, `selectionMode: multiple`, `allowOther: true`, and one Rich Media summary panel. The shared projector keeps Output user-owned, infers each `candidateOption` edge, and applies multi-selection plus Other to the visible Canvas. Provider, MCP, and no-model cards pass the same relevance and cross-card diversity validators: every card needs a different user-named focus, while a repeated question, reused choice label, or reused/subset/superset choice set fails closed. The no-model path derives its choices from authored enumerations and never copies recalled exemplar or fixture content.
