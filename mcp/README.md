...
7. `knowgrph.html_video.render`
   ...
   - A Widget Card containing `/knowgrph.probe-tree`, `@knowgrph.probe-tree`, or `#knowgrph.probe-tree` calls the local stdio `knowgrph.probe.generate` tool through the bounded Dev bridge before optional provider refinement. The active-graph projector accepts only a bounded response whose questions, choices, and verbatim context anchors remain relevant to the selected user input, replaces stale candidates, infers their `candidateOption` edges, and atomically publishes the cards plus owned Rich Media ledger; the deterministic local branch materializer remains a toolbar preview/fallback
