...
7. `knowgrph.html_video.render`
   ...
   - Probe-Tree chat/model responses use `probe-tree-llm-response/v1` under `response.structuredContent`: one source Widget anchor, 2-4 editable cards with `parentNodeId`, and one Rich Media summary panel; each card renders the model-generated `question` as Summary and reserves an initially empty Output for the user answer, while the shared response projector infers each `candidateOption` edge and applies the resulting KGC to the visible Canvas
   ...
   - A Widget Card containing `/knowgrph.probe-tree`, `@knowgrph.probe-tree`, or `#knowgrph.probe-tree` calls the local stdio `knowgrph.probe.generate` tool through the bounded Dev bridge before optional provider refinement. The active-graph projector accepts only a relevant bounded response, replaces stale candidates, infers their `candidateOption` edges, and atomically publishes the cards plus owned Rich Media ledger; the deterministic local branch materializer remains a toolbar preview/fallback
