...
7. `knowgrph.html_video.render`
   ...
   - Probe-Tree chat/model responses use `probe-tree-llm-response/v3` under `response.structuredContent`: one source Widget anchor, 2-4 Type 2 cards with `parentNodeId`, 2-4 numbered suggested clarification answers in `selectionOptions`, 2-6 verbatim `contextAnchors` copied from the selected child or bounded ancestor lineage, `selectionMode: multiple`, `allowOther: true`, and one Rich Media summary panel. The selected child replaces any same-ID stale graph alias before the shared projector infers `candidateOption` edges; the thread root remains lineage-only. Provider and MCP cards pass the same relevance and cross-card diversity validators: repeated questions, bare focus fragments, reused choice labels, and reused/subset/superset choice sets fail closed. The no-model path accepts one exception: a selected child that literally authors 2-4 alternatives projects as one Type 2 card with the question and choice labels preserved verbatim and `probeTreeDerivation: authored-choices`. It otherwise returns no cards and never copies recalled exemplar or fixture content.
   ...
   - A Widget Card containing `/knowgrph.probe-tree`, `@knowgrph.probe-tree`, or `#knowgrph.probe-tree` calls the local stdio `knowgrph.probe.generate` tool through the bounded Dev bridge before optional provider refinement. The active-graph projector accepts only a bounded response whose questions, choices, and verbatim context anchors remain relevant to the selected user input, replaces stale candidates, infers their `candidateOption` edges, and atomically publishes the cards plus owned Rich Media ledger. The bubble toolbar only reveals accepted model-backed branches; it never materializes deterministic fallback cards
...
10. Probe-tree tools
   - `knowgrph.probe.generate` recalls scoped resolved-path exemplars and returns 2-4 typed candidate next questions without mutating the current node; `token_budget` is enforced before a local model call, trimming recalled exemplars first, and `recall_top_k: 0` disables recall explicitly. If fewer than 2 query-relevant model cards survive, the tool may return one `authored_choice_projection` only for a literal 2-4-alternative list. All other no-model requests fail closed with `insufficient_user_input_context` instead of synthesizing generic wrapper cards or converting named entities into templates
   ...
   - `knowgrph.probe.generate` can call a host-owned local Ollama runtime when `KNOWGRPH_PROBE_TREE_MODEL` is set; it uses non-streaming structured JSON output and rejects canned relationship/evidence/dependency/decision-order templates. When the adapter is unconfigured or fails, the no-model path either projects one literal authored-choice card or returns a degraded failure; it never synthesizes clarification axes
