...
7. `knowgrph.html_video.render`
   ...
   - Probe-Tree chat/model responses use `probe-tree-llm-response/v2` under `response.structuredContent`: one source Widget anchor, 2-4 Type 2 cards with `parentNodeId`, 2-4 numbered `selectionOptions`, `selectionMode: multiple`, `allowOther: true`, and one Rich Media summary panel. The shared projector keeps Output user-owned, infers each `candidateOption` edge, and applies multi-selection plus Other to the visible Canvas.
